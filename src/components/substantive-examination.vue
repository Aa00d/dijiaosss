...
const submitDelete = () => {
  ...
  const deleteUrl = `${import.meta.env.VITE_API_BASE_URL}/substantive-examination/delete?case_processes_id=${deleteForm.case_processes_id}&case_id=${deleteForm.case_id}`;
...
const saveExaminationRequest = async () => {
  try {
    ...
    // 构建请求URL - 使用固定的ID值
    const baseUrl = `${import.meta.env.VITE_API_BASE_URL}/substantive-examination/save`;
    const urlParams = getParamsFromUrl();
...
const queryById = async (processId?: string, caseId?: string) => {
  try {
    ...
    // 构建查询URL
    const url = `${import.meta.env.VITE_API_BASE_URL}/substantive-examination/by-case?case_processes_id=${encodeURIComponent(case_processes_id)}&case_id=${encodeURIComponent(case_id)}`;
