...
export async function fetchCaseSummary(id) {
  try {
    const res = await request.get(`/case/${id}/summary`);
    // 支持直接对象或 { data: { ... } } 包裹
...
export function normalizeCaseSummary(summary) {
  ...
  return {
    ...
    reviewType:
      summary.preliminary_case === 1 ||
      summary.preliminary_case === true ||
      summary.preliminary_case === "1" ||
      summary.preliminary_case === "true"
        ? "pre"
        : "",
    priorityExamination:
      summary.priority_examination === true ||
      summary.priority_examination === 1 ||
      summary.priority_examination === "1" ||
      summary.priority_examination === "true",
