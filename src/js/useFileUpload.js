 ...
 */
export const useFileUpload = async ({ files, case_processes_id, case_id, submission_page, special }) => {
  try {
    // 验证必填参数
    if (!files) {
    ...
    // 发送请求
    const response = await fetch('http://q7e69a6c.natappfree.cc/api/files/upload', {
      method: 'POST',
      body: formData,
