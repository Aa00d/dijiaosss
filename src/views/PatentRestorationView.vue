...
// 恢复专利请求相关API
const getPatentRestoration = (id: string) => apiCall(`/restore-rights/${id}`, {
  id: 0, createTime: '', updateTime: '', submissionType: '',
  regulationBasis: '{}', restoreRightsRequest: false, legitimateReason: false,
  forceMajeureReason: false, requestRecovery: '', reasonDescription: '',
  proofFileRecordNumber: '', requestType: ''
})
...
const convertDateFormat = (dateStr: string, targetFormat: 'YYYY-MM-DD' | 'YYYY/MM/DD' = 'YYYY-MM-DD'): string => {
  if (!dateStr) return ''
  // 如果已经是目标格式，直接返回
...
const loadCaseSummaryById = async (id: number | string = 1001) => {
  try {
    ...
    if (info?.priorityExamination === 1 || info?.priorityExamination === true) flags.push('priority')
...
watch(() => patentRestorationData.requestType, (val) => {
...
// 已转档文件数据
const processedFiles = ref<Array<{
  index: number
  fileName: string
  fileType: string
  fileTitle: string
...
const fetchSubmissionFilesOnce = async (caseProcessesId: string, caseId: string): Promise<any[]> => {
  ...
  const fetchByLabel = async (submissionPageLabel: string) => {
    ...
    const url = `http://q7e69a6c.natappfree.cc/api/files/list-by-submission?${queryParams.toString()}`
...
const loadProcessedFiles = async (caseProcessesId: string, caseId: string) => {
  try {
    ...
    processedFiles.value = filesData.map((file: any, index: number) => ({
      index: index + 1,
      fileName: file.file_name || file.fileName || file.original_filename || file.attachment_name || file.name || '',
      fileType: file.file_type || file.fileType || file.file_sub_type || file.file_subcategory || file.fileCategory || file.fileCategoryMinor || '',
      fileTitle: file.file_title || file.fileTitle || file.file_description || file.fileDescription || file.file_name || file.fileName || '',
      fileShortName: file.file_short_name || file.fileShortName || file.file_abbreviation || file.fileAbbreviation || '',
      uploader: file.uploader || file.upload_person || file.uploader_name || file.uploadPerson || file.creator || '',
      uploadTime: file.upload_time || file.uploadTime || file.upload_date || file.uploadDate || file.createTime || file.create_time || '',
      id: file.id || file.file_id || null,
      url: file.url || file.file_url || file.download_url || file.downloadUrl || null
    }))
    console.log('✅ 已转档文件列表加载成功（单次请求缓存），共', processedFiles.value.length, '个文件')
    return
  } catch (error: any) {
...
const loadPendingFiles = async (caseProcessesId: string, caseId: string) => {
  try {
    ...
    console.log('✅ 待转档文件列表加载成功（单次请求缓存），共', pendingFiles.value.length, '个文件')
    return
...
const downloadProcessedFile = async (fileRecord: any) => {
  try {
    ...
    const isOssUrl = downloadUrl.includes('aliyuncs.com') || downloadUrl.includes('oss-') || downloadUrl.includes('amazonaws.com')
...
const viewProcessedFile = async (fileRecord: any) => {
  try {
    ...
    // 获取文件名
    const fileName = fileRecord.fileName || fileRecord.file_name || fileRecord.original_filename || fileUrl.split('/').pop() || '文件'
    ...
    try {
      ...
      if (isSameOrigin) {
        try {
          ...
          if (testResponse.ok || testResponse.status === 304) {
            ...
            if (testResponse.status === 304) console.log('ℹ️ 服务器返回 304（未修改），使用缓存版本')
            if (!contentType.includes('pdf') && !contentType.includes('image') && !contentType.includes('octet-stream') && !contentType.includes('application')) {
      ...
      const urlWithTimestamp = isSameOrigin
        ? (fileUrl.includes('?') ? `${fileUrl}&_t=${Date.now()}` : `${fileUrl}?_t=${Date.now()}`)
        : fileUrl
...
// 上传弹窗相关状态
const uploadAccept = ref('.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document')
...
const loadPatentRestorationData = async (id: string) => {
  try {
    ...
    if (data?.id) {
      ...
      patentRestorationData.submissionType = String(data.submissionType ?? data.submission_type ?? '')
      patentRestorationData.regulationBasis = String(data.regulationBasis ?? data.regulation_basis ?? '{}')
      patentRestorationData.restoreRightsRequest = !!(data.restoreRightsRequest ?? data.restore_rights_request)
      patentRestorationData.legitimateReason = !!(data.legitimateReason ?? data.legitimate_reason)
      patentRestorationData.forceMajeureReason = !!(data.forceMajeureReason ?? data.force_majeure_reason)
      patentRestorationData.requestRecovery = String(data.requestRecovery ?? data.request_recovery ?? '')
      patentRestorationData.reasonDescription = String(data.reasonDescription ?? data.reason_description ?? '')
      patentRestorationData.proofFileRecordNumber = String(data.proofFileRecordNumber ?? data.proof_file_record_number ?? '')
      ...
      try { regulationBasisData = JSON.parse(basisStr) } catch { regulationBasisData = {} }
...
const saveData = async () => {
  try {
    ...
    // 构建查询字符串与接口地址
    const queryParams = `case_processes_id=${encodeURIComponent(String(caseProcessesId))}` +
    ...
    const buildRegulationBasis = () => {
      let base: any = {}
      try { base = patentRestorationData.regulationBasis ? JSON.parse(patentRestorationData.regulationBasis) : {} } catch { base = {} }
      ...
      if (!base.content && caseInfo.applicationNumber) base.content = `根据专利法实施细则相关规定，针对申请号为${caseInfo.applicationNumber}的申请`
      // 可选：通知书名称
    ...
    const requestBody: any = {
      ...
      request_recovery: ensureString(caseInfo.requestRecovery || patentRestorationData.requestRecovery),
...
const onStartXmlConversion = async () => {
  ...
  try {
    ...
    const recoverData = {
      data: petitionData?.noticeDate || petitionData?.notice_date || caseInfo.issueDate || new Date().toISOString().split('T')[0], // 专利局发出通知的日期
      name: petitionData?.noticeName || petitionData?.notice_name || patentRestorationData.requestRecovery || '视为撤回通知书', // 专利局发出通知的名称
      number: petitionData?.documentNumber || petitionData?.document_number || petitionData?.number || caseInfo.applicationNumber || 'CH20250905008', // 发文序号
      select: petitionData?.select !== undefined ? Boolean(petitionData.select) : (petitionData?.isForceMajeure !== undefined ? Boolean(petitionData.isForceMajeure) : patentRestorationData.legitimateReason), // 选择（0为正当理由，1为不可抗力）
      reason: petitionData?.reason || petitionData?.recoveryReason || petitionData?.recovery_reason || patentRestorationData.reasonDescription || '因申请人突发疾病住院治疗，未能在指定期限内答复审查意见，属于不可抗力情形', // 请求恢复权利的理由
      recordFilingNumber: petitionData?.recordFilingNumber || petitionData?.record_filing_number || petitionData?.filingNumber || patentRestorationData.proofFileRecordNumber || 'BH20251020001' // 备案编号
    }
...
const executeIdQuery = async () => {
  ...
  try {
    ...
    if (record && (record.id || Object.keys(record).length > 0)) {
      ...
      patentRestorationData.submissionType = String(record.submissionType ?? record.submission_type ?? '')
      patentRestorationData.regulationBasis = String(record.regulationBasis ?? record.regulation_basis ?? '{}')
      patentRestorationData.restoreRightsRequest = !!(record.restoreRightsRequest ?? record.restore_rights_request)
      patentRestorationData.legitimateReason = !!(record.legitimateReason ?? record.legitimate_reason)
      patentRestorationData.forceMajeureReason = !!(record.forceMajeureReason ?? record.force_majeure_reason)
      patentRestorationData.requestRecovery = String(record.requestRecovery ?? record.request_recovery ?? '')
      patentRestorationData.reasonDescription = String(record.reasonDescription ?? record.reason_description ?? '')
      patentRestorationData.proofFileRecordNumber = String(record.proofFileRecordNumber ?? record.proof_file_record_number ?? '')
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="提案名称:">
              <el-input v-model="caseInfo.proposalName" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="案件号:">
              <el-input v-model="caseInfo.caseNumber" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="申请号:">
              <el-input v-model="caseInfo.applicationNumber" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="案件名称:">
              <el-input v-model="caseInfo.caseName" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item label="处理事项:">
              <el-input v-model="caseInfo.processItem" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="发文日:">
              <el-input v-model="caseInfo.issueDate" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="官方期限:">
              <el-input v-model="caseInfo.officialDeadline" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="内部期限:">
              <el-input v-model="caseInfo.internalDeadline" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="客户名称:">
              <el-input v-model="caseInfo.clientName" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="技术主导:">
              <el-input v-model="caseInfo.techLead" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="双报案件:">
              <el-input v-model="caseInfo.doubleReport" placeholder="单行输入" disabled readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item>
          ...
          <div class="tab-content">
              ...
              <el-row :gutter="20">
                <el-col :span="6">
                  <el-form-item label="递交类型:">
                    <el-select v-model="patentRestorationData.submissionType" placeholder="后台配置选择">
                      <el-option label="后台配置选择" value=""></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
            ...
            <div class="section">
              ...
              <div class="form-row" style="margin-top: 10px;">
                <el-checkbox v-model="patentRestorationData.restoreRightsRequest">主动请求恢复</el-checkbox>
              </div>
              <h4>请求恢复权利的理由</h4>
              <div class="form-row" style="margin-top: 15px;">
                <el-select v-model="patentRestorationData.requestType" placeholder="请求恢复情况" style="width: 280px; margin-right: 20px;" @change="handleRequestTypeChange">
                  <el-option v-for="opt in requestTypeOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
                </el-select>
                <el-radio-group v-model="patentRestorationData.legitimateReason" @change="handleReasonChange">
              ...
              <div class="form-row" style="margin-top: 15px;">
                <div style="display: flex; gap: 12px; align-items: flex-start; width: 100%;">
                  <el-input
                    ...
                    style="flex: 1;"
                  />
                  <el-button type="default" @click="handleRequestTypeChange(patentRestorationData.requestType)" style="white-space: nowrap;">恢复默认模板</el-button>
                </div>
              </div>

              <div class="form-row" style="margin-top: 15px;">
                <el-select v-model="additionalFileType" placeholder="请选择文件类型" style="width: 200px;" @change="handleAdditionalFileTypeChange">
        ...
        <el-tab-pane label="待转档文件" name="pending-content">
          <div class="tab-content">
            <div style="margin-bottom: 20px;">
              <el-button type="success" @click="onStartXmlConversion" :loading="submitLoading">启动转档XML</el-button>
            </div>
            <el-table :data="pendingFiles" style="width: 100%">
              ...
              <el-table-column label="操作" width="120">
                <template #default="{ row }">
                  <el-button link type="danger" size="small" @click="deletePendingFile(row)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div class="tab-content">
            ...
            <el-table :data="processedFiles" style="width: 100%">
              ...
              <el-table-column label="操作" width="180">
                <template #default="{ row }">
                  <el-button  type="primary" size="small" @click="downloadProcessedFile(row)" plain>下载</el-button>
                  <el-button
                    v-if="!String(row.fileName || '').toLowerCase().endsWith('.zip')"
                    type="success"
                    size="small"
