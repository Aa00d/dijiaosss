...
import { getInternalCodeByFileType, FILE_TYPE_INTERNAL_CODE_MAP, getFileTypeOptions } from '../js/InternalCode.js'
import PdfViewer from '../components/PdfViewer.vue'
import { usePdfViewer } from '../js/usePdfViewer.js'
import { useUploadZipBytes } from '../js/useUploadZipBytes.js'

// =================== 新增/调整：API 常量 ===================
const API_BASE_URL = 'http://8.140.210.30:8082/api'       // 你项目已有
// 上传和查询接口：直接使用后端服务器地址，不使用本地代理
// 注意：现在使用带内部代码的上传接口
...
const opinionOptions = ref([
  { value: 'notice_response', label: '针对国家知识产权局于', hasDate: true, hasInput: true, inputPlaceholder: '带出来的通知书（发文序号）' },
...
const updateParsedData = () => {
  ...
  if (agentInfo.value && (agentInfo.value as any).agent_1) {
    ...
    agent1.name = d.name || ''; agent1.license = d.license || ''; agent1.phone = d.phone || ''; agent1.authority = d.authority || ''
  }
  if (agentInfo.value && (agentInfo.value as any).agent_2) {
    const d = (agentInfo.value as any).agent_2
    agent2.name = d.name || ''; agent2.license = d.license || ''; agent2.phone = d.phone || ''; agent2.authority = d.authority || ''
  }
...
function triggerImagesUpload() { imagesInputRef.value?.click() }
...
function triggerComparisonUpload() { comparisonInputRef.value?.click() }
...
function triggerTextUpload() { textFileInputRef.value?.click() }
...
function triggerAttachmentUpload() { attachmentInputRef.value?.click() }
// 触发委托书扫描件上传
function triggerPowerOfAttorneyUpload() { powerOfAttorneyInputRef.value?.click() }
...
const buildUploadedFilePayload = (file: any, internalCode: string, fallbackType = ''): UploadedFilePayload | null => {
  ...
  return { fileUrl: fileUrl || '', fileType: fileType || '', internalCode: internalCode || undefined, fileId }
}
...
async function queryProcessedFiles() {
  ...
  try {
    ...
    processedFiles.value = filteredFileList.map((file: any, index: number) => ({
      ...
      original_filename: file.fileName || file.file_name || file.original_filename || file.name || '',
      ...
      upload_time: file.createTime ? new Date(file.createTime).toLocaleString('zh-CN') : (file.upload_time || file.uploadTime || ''),
  ...
  try {
    ...
    fileList.forEach((file: any) => {
      ...
      const fileData = {
        ...
        sequence: '',  // 序号稍后设置
        original_filename: file.fileName || file.original_filename || file.file_name || file.name || '',
  ...
  if (filtered.length > 0) {
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      if (result) {
        ...
        const fileUrl = extractFileUrl(result) || result.url || result.fileUrl || result.file_url || ''
        if (fileUrl) {
    ...
    if (successCount > 0) {
      ElMessage.success(`上传完成：成功 ${successCount} 个${successCount < filtered.length ? `，失败 ${filtered.length - successCount} 个` : ''}`)
      // 上传成功后自动查询文件列表刷新表格
      await queryFiles()
    } else {
...
async function handlePowerOfAttorneySelected(e: Event) {
  ...
  if (filtered.length < files.length) {
    ElMessage.warning(`已过滤 ${files.length - filtered.length} 个不支持的文件，委托书扫描件只能上传JPG/JPEG格式`)
  }
...
function buildStatementRecheckString() {
  ...
  const statementopinions = [
    { representor: 1, name: formData.applicantName || '', phone: formData.applicantNamePhone || '' },
    { representor: 2, name: formData.invalidApplicantName || '', phone: formData.invalidApplicantPhone || '' }
  ].filter(op => op.name || op.phone)

  const agentDtos = [
    { name: agent1.name || '', sort: 1, qualificationCertificate: agent1.license || '', phone: agent1.phone || '', agency: true },
    { name: agent2.name || '', sort: 2, qualificationCertificate: agent2.license || '', phone: agent2.phone || '', agency: false }
  ...
  const payload = {
    ...
    notification: { name: reviewOpinion.value || '', serialNumber: noticeNumber.value || '', Data: toDateString(targetDate.value) },
...
const onSubmit = async () => {
  try {
    ...
    const latestFilesPayload = latestUploadedFiles.value.filter(item => item.fileUrl && item.fileType)
    ...
    comparisonPageUrls.value.forEach(({ url, name }, idx) => {
      const n = (name || '').trim()
      const finalName = n || url.split('/').pop()?.replace(/\.(pdf|docx?)$/i, '') || '附件'
    ...
    form.append('internet_evidence', JSON.stringify({
      enabled: !!internetEvidence.value,
      evidence_name: evidenceName.value,
      evidence_number: evidenceNumber.value
    }))
    ...
    try {
      ...
      for (const [key, value] of (form as any).entries()) {
        formDataSummary[key] = value instanceof File ? value.name : (typeof value === 'string' ? (key === 'docx' ? (value.length > 80 ? value.slice(0, 80) + '…' : value) : value) : value)
      }
    ...
    if (resp.ok) {
      if ((contentType && contentType.includes('application/zip')) || (contentType && contentType.includes('application/octet-stream'))) {
        ...
        try {
          ...
          ElMessage.error(`上传到数据库失败：${uploadErr instanceof Error ? uploadErr.message : '未知错误'}`)
          throw uploadErr
        }
      } else {
...
const onReturn = () => { ElMessage.info('退回') }
const onTransfer = () => { ElMessage.info('移交') }
...
const closeDeleteDialog = () => { showDeleteDialog.value = false; deleteForm.case_processes_id = ''; deleteForm.case_id = '' }
const onDelete = async () => {
  try {
    const { case_id, case_processes_id } = idsForActions()
    if (!case_id || !case_processes_id) { ElMessage.error('缺少案件ID或处理事项ID，无法删除'); return }
    ...
    if (e && (e === 'cancel' || e === 'close')) { ElMessage.info('已取消删除') }
    else { console.error('删除错误:', e); ElMessage.error('删除失败') }
  }
}
const confirmDelete = async () => {
  if (!deleteForm.case_processes_id || !deleteForm.case_id) { ElMessage.warning('请输入处理事项ID和案件ID'); return }
  try { await performDelete(deleteForm.case_processes_id, deleteForm.case_id); closeDeleteDialog() }
  catch (error) { console.error('删除失败:', error); ElMessage.error('删除失败，请重试') }
}

const onQueryAll = () => { ElMessage.info('查询所有') }
...
const closeIdQueryModal = () => { showIdQueryModal.value = false; idQueryForm.caseId = ''; idQueryForm.caseProcessesId = '' }
const executeIdQuery = async () => {
  if (!idQueryForm.caseId.trim()) { ElMessage.warning('请输入案件ID'); return }
  if (!idQueryForm.caseProcessesId.trim()) { ElMessage.warning('请输入处理事项ID'); return }
  try {
...
// ====== 按钮行为（修改了"添加附件"） ======
const addOpinionStatement = () => { ElMessage.info('添加复审无效意见陈述') }
const addAttachment = () => { triggerAttachmentUpload() } // ← 直接弹文件选择并走上传
...
const deleteFile = async (index: number) => {
  ...
  if (!fileId) {
    ...
    fileTableData.value.forEach((item, idx) => { item.sequence = (idx + 1).toString() })
    ElMessage.success('删除成功')
    return
  }
  ...
  try {
    ...
    await ElMessageBox.confirm(
      `确认删除文件 "${file.original_filename}" 吗？`,
      '删除确认',
      {
  ...
  try {
    ...
    if (!fileId) {
      ...
      ElMessageBox.confirm(
        ...
        evidenceTableData.value.forEach((item, idx) => { item.sequence = (idx + 1).toString() })
        ElMessage.success('删除成功')
      }).catch(() => {})
      return
    }
...
const loadData = async (caseId: number, caseProcessesId: number) => {
  ...
  if (!caseId || !caseProcessesId || !Number.isFinite(caseId) || !Number.isFinite(caseProcessesId)) {
    return
  }
  ...
  try {
    ...
    if (result.success && result.data && result.data.length > 0) {
      ...
      if (data.createTime !== null && data.createTime !== undefined) formData.createTime = data.createTime
      if (data.updateTime !== null && data.updateTime !== undefined) formData.updateTime = data.updateTime
      if (data.submissionType !== null && data.submissionType !== undefined) formData.submissionType = data.submissionType
      if (data.applicantType !== null && data.applicantType !== undefined) formData.applicantType = data.applicantType
      if (data.applicantName !== null && data.applicantName !== undefined) formData.applicantName = data.applicantName
      if (data.applicantNamePhone !== null && data.applicantNamePhone !== undefined) formData.applicantNamePhone = data.applicantNamePhone
      if (data.invalidApplicantName !== null && data.invalidApplicantName !== undefined) formData.invalidApplicantName = data.invalidApplicantName
      if (data.invalidApplicantPhone !== null && data.invalidApplicantPhone !== undefined) formData.invalidApplicantPhone = data.invalidApplicantPhone
      if (data.caseNumber !== null && data.caseNumber !== undefined) formData.caseNumber = data.caseNumber
      if (data.dateInfo !== null && data.dateInfo !== undefined) formData.dateInfo = data.dateInfo
      if (data.submitReviewInvalidOpinion !== null && data.submitReviewInvalidOpinion !== undefined) formData.submitReviewInvalidOpinion = data.submitReviewInvalidOpinion
      if (data.submitReviewInvalidDate !== null && data.submitReviewInvalidDate !== undefined) formData.submitReviewInvalidDate = data.submitReviewInvalidDate
      if (data.submitContactAddressChange !== null && data.submitContactAddressChange !== undefined) formData.submitContactAddressChange = data.submitContactAddressChange
      if (data.submitAuthorizationLetter !== null && data.submitAuthorizationLetter !== undefined) formData.submitAuthorizationLetter = data.submitAuthorizationLetter
      if (data.submitOralHearing !== null && data.submitOralHearing !== undefined) formData.submitOralHearing = data.submitOralHearing
      if (data.requestCombinedReview !== null && data.requestCombinedReview !== undefined) formData.requestCombinedReview = data.requestCombinedReview
      if (data.withdrawReviewInvalidRequest !== null && data.withdrawReviewInvalidRequest !== undefined) formData.withdrawReviewInvalidRequest = data.withdrawReviewInvalidRequest
      if (data.aboutFees !== null && data.aboutFees !== undefined) formData.aboutFees = data.aboutFees
      if (data.opinion !== null && data.opinion !== undefined) formData.opinion = data.opinion
      if (data.attachmentDocuments !== null && data.attachmentDocuments !== undefined) formData.attachmentDocuments = data.attachmentDocuments
      if (data.attachmentType !== null && data.attachmentType !== undefined) formData.attachmentType = data.attachmentType
      if (data.internetEvidence !== null && data.internetEvidence !== undefined) formData.internetEvidence = data.internetEvidence
      if (data.agencyName !== null && data.agencyName !== undefined) formData.agencyName = data.agencyName
      if (data.agencyCode !== null && data.agencyCode !== undefined) formData.agencyCode = data.agencyCode
      if (data.invalidProgram !== null && data.invalidProgram !== undefined) formData.invalidProgram = data.invalidProgram
      if (data.agentInfo !== null && data.agentInfo !== undefined) formData.agentInfo = data.agentInfo
      if (data.powerOfAttorneyAttachments !== null && data.powerOfAttorneyAttachments !== undefined) formData.powerOfAttorneyAttachments = data.powerOfAttorneyAttachments
      if (data.commissionDate !== null && data.commissionDate !== undefined) formData.commissionDate = data.commissionDate
      if (data.applicantAddress !== null && data.applicantAddress !== undefined) formData.applicantAddress = data.applicantAddress
      if (data.agencyAddress !== null && data.agencyAddress !== undefined) formData.agencyAddress = data.agencyAddress
      if (data.applicantPostal !== null && data.applicantPostal !== undefined) formData.applicantPostal = data.applicantPostal
      if (data.agencyPostal !== null && data.agencyPostal !== undefined) formData.agencyPostal = data.agencyPostal
...
const saveData = async () => {
  try {
    const { case_id, case_processes_id } = idsForActions()
    if (!case_id || !case_processes_id) { ElMessage.error('缺少案件ID或处理事项ID，无法保存'); return }
    ...
    console.log('formData.submissionType:', formData.submissionType, '类型:', typeof formData.submissionType, '是否为null:', formData.submissionType === null)
    console.log('formData.applicantName:', formData.applicantName, '类型:', typeof formData.applicantName, '是否为null:', formData.applicantName === null)
    console.log('formData.applicantNamePhone:', formData.applicantNamePhone, '类型:', typeof formData.applicantNamePhone, '是否为null:', formData.applicantNamePhone === null)
    console.log('formData.invalidApplicantName:', formData.invalidApplicantName, '类型:', typeof formData.invalidApplicantName, '是否为null:', formData.invalidApplicantName === null)
    console.log('formData.invalidApplicantPhone:', formData.invalidApplicantPhone, '类型:', typeof formData.invalidApplicantPhone, '是否为null:', formData.invalidApplicantPhone === null)
    console.log('formData.caseNumber:', formData.caseNumber, '类型:', typeof formData.caseNumber, '是否为null:', formData.caseNumber === null)
    console.log('formData.agencyName:', formData.agencyName, '类型:', typeof formData.agencyName, '是否为null:', formData.agencyName === null)
    console.log('formData.opinion:', formData.opinion, '类型:', typeof formData.opinion, '是否为null:', formData.opinion === null)
    ...
    const requestBody: any = {
      ...
      submission_type: formData.submissionType !== null && formData.submissionType !== undefined ? formData.submissionType : '',
      applicant_type: formData.applicantType !== null && formData.applicantType !== undefined ? formData.applicantType : '',
      applicant_name: formData.applicantName !== null && formData.applicantName !== undefined ? formData.applicantName : '',
      applicant_name_phone: formData.applicantNamePhone !== null && formData.applicantNamePhone !== undefined ? formData.applicantNamePhone : '',
      invalid_applicant_name: formData.invalidApplicantName !== null && formData.invalidApplicantName !== undefined ? formData.invalidApplicantName : '',
      invalid_applicant_phone: formData.invalidApplicantPhone !== null && formData.invalidApplicantPhone !== undefined ? formData.invalidApplicantPhone : '',
      case_number: formData.caseNumber !== null && formData.caseNumber !== undefined ? formData.caseNumber : '',
      date_info: formData.dateInfo !== null && formData.dateInfo !== undefined ? formData.dateInfo : '',
      submit_review_invalid_opinion: formData.submitReviewInvalidOpinion ?? false,
      submit_review_invalid_date: formData.submitReviewInvalidDate !== null && formData.submitReviewInvalidDate !== undefined ? formData.submitReviewInvalidDate : '',
      ...
      attachment_documents: formData.attachmentDocuments !== null && formData.attachmentDocuments !== undefined ? formData.attachmentDocuments : '',
      attachment_type: formData.attachmentType !== null && formData.attachmentType !== undefined ? formData.attachmentType : '',
      internet_evidence: formData.internetEvidence !== null && formData.internetEvidence !== undefined ? formData.internetEvidence : '',
      agency_name: formData.agencyName !== null && formData.agencyName !== undefined ? formData.agencyName : '',
      agency_code: formData.agencyCode !== null && formData.agencyCode !== undefined ? formData.agencyCode : '',
      invalid_program: formData.invalidProgram ?? false,
      agent_info: formData.agentInfo !== null && formData.agentInfo !== undefined ? formData.agentInfo : '',
      power_of_attorney_attachments: formData.powerOfAttorneyAttachments !== null && formData.powerOfAttorneyAttachments !== undefined ? formData.powerOfAttorneyAttachments : '',
      commission_date: formData.commissionDate !== null && formData.commissionDate !== undefined ? formData.commissionDate : '',
      applicant_address: formData.applicantAddress !== null && formData.applicantAddress !== undefined ? formData.applicantAddress : '',
      agency_address: formData.agencyAddress !== null && formData.agencyAddress !== undefined ? formData.agencyAddress : '',
      applicant_postal: formData.applicantPostal !== null && formData.applicantPostal !== undefined ? formData.applicantPostal : '',
      agency_postal: formData.agencyPostal !== null && formData.agencyPostal !== undefined ? formData.agencyPostal : ''
    }
    ...
    Object.keys(requestBody).forEach(key => {
      ...
      console.log(`  ${key}: ${val} (${typeof val}, isNull: ${val === null}, isUndefined: ${val === undefined})`)
    })
...
watch(() => formData.applicantName, (newVal, oldVal) => {
  console.log('formData.applicantName 变化:', oldVal, '->', newVal)
}, { immediate: false })

watch(() => formData.invalidApplicantName, (newVal, oldVal) => {
  console.log('formData.invalidApplicantName 变化:', oldVal, '->', newVal)
}, { immediate: false })

watch(() => formData.caseNumber, (newVal, oldVal) => {
  console.log('formData.caseNumber 变化:', oldVal, '->', newVal)
}, { immediate: false })

watch(() => formData.opinion, (newVal, oldVal) => {
...
const onSave = () => {
  ...
  console.log('  applicantNamePhone:', formData.applicantNamePhone, typeof formData.applicantNamePhone)
  console.log('  invalidApplicantName:', formData.invalidApplicantName, typeof formData.invalidApplicantName)
  console.log('  invalidApplicantPhone:', formData.invalidApplicantPhone, typeof formData.invalidApplicantPhone)
  ...
  <main>
          ...
          <el-col :span="6">
            <el-form-item label="案件状态:">
              <el-select v-model="caseInfo.caseStatus" placeholder="后台配置选择" disabled>
                <el-option :label="caseInfo.caseStatus || '后台配置选择'" :value="caseInfo.caseStatus || ''"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="6">
            <el-form-item label="代理机构:">
              <el-select v-model="caseInfo.agency" placeholder="后台配置选择" disabled>
                <el-option :label="caseInfo.agency || '后台配置选择'" :value="caseInfo.agency || ''"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="6">
            <el-form-item label="业务人员:">
              <el-select v-model="caseInfo.businessPersonnel" placeholder="后台配置选择" disabled>
                <el-option :label="caseInfo.businessPersonnel || '后台配置选择'" :value="caseInfo.businessPersonnel || ''"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="请求书" name="request">
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-form :model="formData" label-width="120px">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="专利权人:">
                  <el-input v-model="formData.applicantName" placeholder="多个专利人,用逗号分隔"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="无效宣告请求人:">
                  <el-input v-model="formData.invalidApplicantName" placeholder="多个请求人,用逗号分隔"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="电话:">
                  <el-input v-model="formData.invalidApplicantPhone" placeholder="单行输入"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-form-item label="案件编号:">
              <el-input v-model="formData.caseNumber" placeholder="单行输入" style="width: 50%;"></el-input>
            </el-form-item>
          </el-form>
        </el-card>
        ...
        <el-card style="margin-bottom: 10px;">
          <template #header>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <input ref="attachmentInputRef" type="file" :accept="attachmentAccept" multiple style="display:none" @change="handleAttachmentSelected" />
          ...
          <div style="display: flex; align-items: center; margin-bottom: 15px;">
            <el-checkbox v-model="internetEvidence" style="margin-right: 15px;">添加互联网证据</el-checkbox>

            <div style="display: flex; align-items: center; margin-right: 15px;">
              <label style="margin-right: 5px;">证据名称:</label>
              <el-input v-model="evidenceName" placeholder="单行输入" style="width: 150px;"></el-input>
            </div>

            <div style="display: flex; align-items: center;">
              <label style="margin-right: 5px;">证据编号:</label>
              <el-input v-model="evidenceNumber" placeholder="单行输入" style="width: 150px;"></el-input>
            </div>
          </div>
          ...
          <el-table :data="fileTableData" style="width: 100%">
            ...
            <el-table-column prop="original_filename" label="上传文件名称" min-width="150"></el-table-column>
            ...
            <el-table-column prop="file_abbreviation" label="文件简称" width="150"></el-table-column>
            ...
            <el-table-column label="操作" width="160">
              <template #default="scope">
                <el-button link type="danger" size="small" @click="deleteFile(scope.$index)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-form :model="formData" label-width="120px">
            <el-form-item>
              ...
              <input ref="powerOfAttorneyInputRef" type="file" accept=".jpg,.jpeg,image/jpeg" multiple style="display:none" @change="handlePowerOfAttorneySelected" />
              <el-button type="primary" style="margin-left: 20px;" @click="triggerPowerOfAttorneyUpload">添加无效委托书扫描件</el-button>
            </el-form-item>
            ...
            <el-table :data="evidenceTableData" style="width: 100%; margin-bottom: 15px;">
              ...
              <el-table-column prop="original_filename" label="上传文件名称" min-width="100"></el-table-column>
              <el-table-column prop="file_subcategory" label="文件小类" width="150"></el-table-column>
              <el-table-column prop="file_description" label="文件名称" width="200"></el-table-column>
              <el-table-column prop="file_abbreviation" label="文件简称" width="250"></el-table-column>
              ...
              <el-table-column label="操作" width="160">
                <template #default="scope">
                  <el-button link type="danger" size="small" @click="deleteEvidence(parseInt(scope.row.sequence))">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
            ...
            <el-row :gutter="20">
              <el-col :span="8">
                <el-form-item label="委托日期:">
                  <el-date-picker v-model="formData.commissionDate" type="date" placeholder="yyyy/mm/日"></el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <template v-if="processedFiles.length">
            <el-table :data="processedFiles" style="width: 100%">
              ...
              <el-table-column prop="fileName" label="文件名" min-width="220">
                <template #default="scope">
                  {{ scope.row.fileName || scope.row.file_name || scope.row.original_filename || '未知文件' }}
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="200">
                <template #default="scope">
                  <el-button  type="primary" size="small" @click="downloadProcessedFile(scope.row)" plain>下载</el-button>
                  <el-button
      ...
      <el-tab-pane label="流程过程" name="process">
        <!-- 已转档文件内容 -->
        <div style="width: 300px; margin-left: 20px; height: 300px; border: 1px solid #ccc; background-color: rgba(0, 0, 0, 0.39); margin-bottom: 20px;">
        </div>
        ...
        <el-table :data="fileTableData" style="width: 100%">
          ...
          <el-table-column label="操作" width="160">
            <template #default="scope">
              <el-button  type="primary" size="small" @click="previewFile(scope.row)" plain>查看</el-button>
