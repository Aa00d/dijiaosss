...
//const API_BASE_URL = 'http://q7e69a6c.natappfree.cc/api'
...
// 请求书案件信息API（按组合键查询）
const getPetitionCaseInfo = (caseProcessesId: string, caseId: string) => apiCall(`/petition/by-case?case_processes_id=${encodeURIComponent(caseProcessesId)}&case_id=${encodeURIComponent(caseId)}`, {
  applicationType: 0,
  institutionCode: '',
  organizationName: ''
})
...
const applyCorrectionToForm = (data: any) => {
  ...
  correctionTempData.value.regulation_50_checked = data.regulation50Checked ?? data.regulation_50_checked ?? false
  correctionTempData.value.regulation_57_checked = data.regulation57Checked ?? data.regulation_57_checked ?? false
  correctionTempData.value.correction_deadline = data.correctionDeadline || data.correction_deadline || ''
  ...
  correctionTempData.value.revision_type = (typeof initialRevisionType === 'string' && initialRevisionType.trim().length === 0)
    ? null
    : initialRevisionType
  correctionTempData.value.filed_document_barcode = data.filedDocumentBarcode || data.filed_document_barcode || ''
  ...
  try {
    correctionItemsInfo.value = typeof correctionItemsStr === 'string' ? JSON.parse(correctionItemsStr) : correctionItemsStr
  } catch (e) {
  ...
  try {
    uploadedFilesInfo.value = typeof uploadedFilesStr === 'string' ? JSON.parse(uploadedFilesStr) : uploadedFilesStr
  } catch (e) {
...
const ensureCombinedKeyInUrl = async () => {
  const hasCp = typeof route.query.case_processes_id === 'string' && route.query.case_processes_id.length > 0
...
const loadFileLists = async () => {
  ...
  try {
    ...
    if (res && typeof res === 'object' && !Array.isArray(res)) {
      caseProcessSubmissionId.value = (res as any).case_process_submission_id ?? (res as any).caseProcessSubmissionId ?? null
        ...
        uploadedFilesList.value.forEach((local: any) => {
          if (!local.server_id) {
            const match = filesArr.find((f: any) => {
              const sameSubmission = (f.case_process_submission_id ?? f.caseProcessSubmissionId ?? null) === caseProcessSubmissionId.value
    ...
    try {
      ...
      const getSpecial = (it: any) => String(it?.special ?? it?.specialFlag ?? it?.special_flag ?? '')
      ...
      const mapped = requestTabFiles.map((f: any, idx: number) => ({
        ...
        original_filename: normalize(f.fileName ?? f.upload_filename ?? f.attachment_name ?? f.original_filename ?? ''),
      ...
      pendingFilesList.value = nonProcessed.map((f: any, idx: number) => ({
        id: Number(f.id),
        sequence: typeof f.sequence === 'number' ? f.sequence : (idx + 1),
        upload_filename: String(normalize(f.upload_filename ?? f.fileName ?? f.attachment_name ?? f.original_filename ?? '')),
        file_sub_type: String(normalize(f.file_sub_type ?? f.fileCategoryMinor ?? f.file_type ?? '')),
        ...
        url: String(
          (f.url || f.file_url || f.fileUrl || f.download_url || f.attachment_url || f.base_url || '')
        )
      }))
      ...
      processedFilesList.value = processed.map((f: any, idx: number) => ({
        id: Number(f.id),
        sequence: typeof f.sequence === 'number' ? f.sequence : (idx + 1),
        attachment_name: String(normalize(f.attachment_name ?? f.upload_filename ?? f.fileName ?? f.original_filename ?? '')),
        file_sub_type: String(normalize(f.file_sub_type ?? f.fileCategoryMinor ?? f.file_type ?? '')),
        file_name: String(normalize(f.file_name ?? f.file_description ?? '')),
        file_short_name: String(normalize(f.file_short_name ?? f.file_description ?? '')),
        uploader: String(f.uploader ?? '未知'),
...
const extractRowFileMeta = (row: any): { url: string; name: string } => {
  ...
  const name = row?.file_name || row?.fileName || row?.upload_filename || row?.original_filename || row?.attachment_name || row?.file_short_name || '文件'
  return { url, name }
}
...
const handleProcessedFileView = async (row: any) => {
  try {
    ...
    const isPdf = /\.pdf$/i.test(String(name)) || /application\/pdf/i.test(String(row?.contentType || ''))
    if (!isPdf) {
...
const isPdfFileRow = (row: any): boolean => {
  try {
    const name = row?.file_name || row?.fileName || row?.upload_filename || row?.original_filename || row?.attachment_name || row?.file_short_name || ''
...
// 标签页状态管理
const activeTab = ref<'request-content' | 'pending-content' | 'processed-content' | 'process-content'>('request-content')
...
const revisionTypeOptions = computed(() => {
  if (correctionTempData.value.revision_mode === 'revision_mode') {
    // 修订模式：只显示修改对照页选项
    return [
      { label: '修改对照页', value: 'comparison_page' }
    ]
  } else {
    // 其他模式：显示四个文档类型选项
    return [
...
watch(() => correctionTempData.value.revision_mode, (newMode) => {
  // 当修订模式改变时，重置文档类型选择
  correctionTempData.value.revision_type = null
...
const loadCorrectionData = async () => {
  try {
    ...
    const responseData = await fetchCorrectionByCase(Number(caseProcessesId.value), Number(caseId.value))
  ...
  correctionTempData.value.revision_type = (typeof rt === 'string' && rt.trim().length === 0) ? null : rt
...
const loadCaseSummaryById = async (id?: string) => {
  try {
    ...
    caseInfo.value.reviewType = summary.reviewType || (summary.priorityExamination === 1 ? 'priority' : '')
...
const correctionItemsInfo = ref<Array<{
  sequence: number,
  document_name: string,
  position_in_document: string,
  correction_before: string,
...
const uploadedFilesInfo = ref<Array<{
  sequence: number,
  original_filename: string,
  file_size: string,
  file_type: string,
...
const latestDeclarationDto = ref<{ url: string, name: string, fileCategoryMinor: string } | null>(null)
...
// 补正内容列表 - 使用新的数据结构
const correctionList = ref<Array<{
  id: number,
  sequence: number,
  document_name: string,
  position_in_document: string,
...
// 上传文件列表 - 使用新的数据结构
const uploadedFilesList = ref<Array<{
  id: number,
...
// 待转档文件列表
const pendingFilesList = ref<Array<{
  id: number,
  sequence: number,
...
const deletePendingFile = async (fileId: number) => {
  try {
    ...
    const filesArr: any[] = Array.isArray(fileList.value) ? fileList.value : []
    console.log('[删除-待转档] 当前组合键:', { case_processes_id: caseProcessesId.value, case_id: caseId.value })
    ...
    const sameSubmissionFiles = filesArr.filter((f: any) => (
      (f.case_process_submission_id ?? f.caseProcessSubmissionId ?? null) === caseProcessSubmissionId.value
    ))
    console.log('[删除-待转档] 同一提交范围内文件数量:', sameSubmissionFiles.length)
    const match = sameSubmissionFiles.find((f: any) => (
      // 直接按服务端ID匹配
      f.id === fileId ||
      // 文件名匹配
      (pending && (
    ...
    await confirmAndDeleteFile(idForDelete, () => {
      ...
      if (idx > -1) {
        ...
        pendingFilesList.value.forEach((f, i) => { f.sequence = i + 1 })
      }
      // 同步刷新服务端文件列表
      loadFileLists().then(() => {
...
const handlePendingFileDownload = async (row: any) => {
  try {
    ...
    let name: string = String(row?.upload_filename || row?.file_name || row?.file_short_name || '文件.zip')

    if (!url) {
      const filesArr: any[] = Array.isArray(fileList.value) ? fileList.value : []
      const targetSubmissionId = caseProcessSubmissionId.value
      const scope = targetSubmissionId
        ? filesArr.filter((f: any) => (
            (f.case_process_submission_id ?? f.caseProcessSubmissionId ?? null) === targetSubmissionId
          ))
        : filesArr
      const match = scope.find((f: any) => {
        const fName = String(f.fileName || f.upload_filename || f.attachment_name || f.original_filename || '')
        return !!fName && (fName === row.upload_filename || fName === row.file_name || fName === row.file_short_name)
      })
      url = String(match?.url || match?.file_url || match?.fileUrl || match?.download_url || match?.attachment_url || match?.base_url || '')
      name = String(match?.file_name || match?.upload_filename || match?.attachment_name || match?.original_filename || name)
    }
...
// 已转档文件列表
const processedFilesList = ref<Array<{
  id: number,
  sequence: number,
...
const switchTab = (tabName: string) => {
  activeTab.value = tabName as 'request-content' | 'pending-content' | 'processed-content' | 'process-content'
}
...
function getFileTypeLabelForMapping(revisionMode: string | null, revisionType: string | null): string {
  // 修订模式：只有“修改对照页”
...
const saveUploadedFile = async () => {
  ...
  try {
    ...
    const typeLabel = mappingLabel || getRevisionTypeLabel(correctionTempData.value.revision_type || undefined)
    ...
    try {
      ...
      const getMinor = (f: any) => (f?.fileCategoryMinor ?? f?.file_sub_type ?? f?.file_type ?? '').toString().trim()
      const getUrl = (f: any) => (f?.url || f?.file_url || f?.fileUrl || f?.download_url || f?.attachment_url || '').toString().trim()
      const matched = filesArr.find((f: any) => (
        (f.id ?? f.file_id ?? f.server_id) === serverIdNum
      )) || null
...
const resolveFilesIdForUploadedRow = (row: any): number | null => {
  try {
    ...
    const match = scope.find((f: any) => {
      const fName = normalize(f.fileName ?? f.upload_filename ?? f.attachment_name ?? f.original_filename)
      ...
      return (
        ...
        (!!rName && fName === rName) ||
        // 小类/描述匹配
        (!!rType && fMinor === rType) || (!!rDesc && (fMinor === rDesc || fDesc === rDesc)) ||
        // 序号匹配
...
const deleteUploadedFile = async (fileId: number | null) => {
  try {
    ...
    const fileIndex = uploadedFilesList.value.findIndex(file => file.id === fileId || file.server_id === fileId)
    if (fileIndex === -1) {
    ...
    if (!serverId) {
      const filesArr: any[] = Array.isArray(fileList.value) ? fileList.value : []
      console.log('[删除-已上传] 当前组合键:', { case_processes_id: caseProcessesId.value, case_id: caseId.value })
    ...
    await confirmAndDeleteFile(serverId, () => {
      ...
      const infoIndex = uploadedFilesInfo.value.findIndex(file => file.sequence === fileToDelete.sequence)
      if (infoIndex > -1) {
        uploadedFilesInfo.value.splice(infoIndex, 1)
      }
      const groupIndex = fileNameDtoGroups.value.findIndex(group => group.fileCategoryMinor === fileToDelete.file_description)
      if (groupIndex > -1) {
        fileNameDtoGroups.value.splice(groupIndex, 1)
      }
      uploadedFilesList.value.forEach((file, idx) => { file.sequence = idx + 1 })
      uploadedFilesInfo.value.forEach((file, idx) => { file.sequence = idx + 1 })
      // 刷新服务端文件列表
      loadFileLists().then(() => {
...
const deleteProcessedFile = async (row: any) => {
  try {
    ...
    await confirmAndDeleteFile(serverId, async () => {
      ...
      processedFilesList.value.forEach((f, i) => { (f as any).sequence = i + 1 })
      // 刷新服务端文件列表
      await loadFileLists()
      loading.close()
...
const saveAmendmentContent = () => {
  const { document_name, position_in_document, correction_before, correction_after } = amendmentForm.value
...
const deleteCorrection = async (sequence: number) => {
  try {
    ...
    if (index > -1) {
      ...
      if (deletedItem) {
        const infoIndex = correctionItemsInfo.value.findIndex(info => info.sequence === deletedItem.sequence)
        if (infoIndex > -1) {
      ...
      // 重新排序（保持 sequence 连续）
      correctionList.value.forEach((i, idx) => { i.sequence = idx + 1 })
      correctionItemsInfo.value.forEach((i, idx) => { i.sequence = idx + 1 })
...
const handleSave = async () => {
  try {
    ...
    // 保存成功后自动刷新数据
    try {
      const refreshData = await fetchCorrectionByCase(Number(caseProcessesId.value), Number(caseId.value))
...
const handleDelete = async () => {
  try {
    ...
    console.log('开始删除补正数据，参数:', { caseProcessesId: caseProcessesId.value, caseId: caseId.value })

    // 调用删除API
    const response = await deleteCorrectionByCase(Number(caseProcessesId.value), Number(caseId.value))
...
function getRevisionTypeLabel(val: string | undefined): string {
  switch (val) {
    case 'document_type_1': return '文档类型1'
    case 'document_type_2': return '文档类型2'
    case 'document_type_3': return '文档类型3'
    default: return val || ''
  }
}
...
watch(() => correctionTempData.value.revision_type, (val: any) => {
...
const removeFileNameDtoGroup = (idx: number) => {
  ...
  if (fileNameDtoGroups.value.length === 0) fileNameDtoGroups.value.push({ file: null, url: '', fileCategoryMinor: '' })
}
const onGroupFileChange = (e: Event, idx: number) => {
  ...
  if (f && fileNameDtoGroups.value[idx]) {
    ...
    const currentTypeLabel = getRevisionTypeLabel(correctionTempData.value.revision_type || undefined)
    if (!fileNameDtoGroups.value[idx].fileCategoryMinor) fileNameDtoGroups.value[idx].fileCategoryMinor = currentTypeLabel || f.name
...
async function uploadZipFromUrl(zipUrl: string) {
  ...
  try {
    ...
    const BIN_UPLOAD_BASE = 'http://8.140.210.30:8082'
    ...
    if (postResp.ok && (postJson?.success === true || postJson?.code === 200)) {
      ...
      try {
        ...
        const pick = items.find((it: any) => /声明/.test(String(it?.file_category_minor || it?.file_name || ''))) || items[0]
        if (pick) {
          latestDeclarationDto.value = {
      ...
      console.error('ZIP URL 转发失败:', { status: postResp.status, statusText: postResp.statusText, body: postText })
...
async function submitAdditionsCorrections() {
  try {
    ...
    const getSpecialLocal = (it: any) => String(it?.special ?? it?.specialFlag ?? it?.special_flag ?? '')
    ...
    const dtoFromUploadMgr = declared.map((f: any) => ({ url: getUrl(f), fileCategoryMinor: '声明文件' })).filter(d => !!d.url && !!d.fileCategoryMinor)
    ...
    const groupsFallback = fileNameDtoGroups.value.filter(g => !!g.url && !!g.fileCategoryMinor).map(g => ({ url: g.url, fileCategoryMinor: g.fileCategoryMinor, name: (g as any).name || '声明文件' }))
    const baseDtoList = dtoFromLatest.length > 0 ? dtoFromLatest : (dtoFromUploadMgr.length > 0 ? dtoFromUploadMgr : groupsFallback)
    ...
    const special1DtosRaw = filesArr
      ...
      .map((f: any) => ({
        ...
        name: (f?.file_name ?? f?.file_description ?? f?.upload_filename ?? f?.attachment_name ?? '附件文件').toString()
      }))
      .filter(d => !!d.url)
    ...
    const businessTypeVal = typeof petitionData.value?.applicationType === 'number'
    ...
    const signatureVal = petitionData.value?.organizationName || caseInfo.value.agency || '成都睿道智诚专利代理有限公司'
    const mysqlPayload = {
      date: deadline,
    ...
    console.log('FormData 字段预览: MysqlString=', !!statementSqlString.value, ', correctionLetterString=', !!correctionLetterString.value, ', fileNameDto条目数=', dtoList.length)
    try {
      console.table(dtoList.map((d, i) => ({ idx: i, url: d.url, fileCategoryMinor: d.fileCategoryMinor })))
    } catch {}
    ...
    if (resp.ok) {
      ...
      const isZip = /zip|octet-stream/i.test(contentType) || /\.zip/i.test(contentDisposition) || isZipArrayBuffer(buffer)
      if (isZip) {
        ...
        try {
          if (!caseProcessesId.value || !caseId.value) {
            ...
            let binJson: any = {}
            try { binJson = JSON.parse(binText) } catch {}
            if (binResp.ok && (binJson?.success === true || binJson?.code === 200)) {
              ...
              console.error('ZIP二进制上传失败:', { status: binResp.status, statusText: binResp.statusText, body: binText })
        ...
        try {
          ...
          if (typeof zipUrlCandidate === 'string' && /\.zip(\?|$)/i.test(zipUrlCandidate)) {
            ...
            ElMessage.error(`响应非ZIP(${contentType || '未知类型'})，且未包含zip_url；请检查后端返回格式`)
          }
        } catch (e) {
...
<main>
 <h2>补正</h2>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="提案名称:">
            <el-input v-model="caseInfo.proposalName" disabled readonly></el-input><!-- 案件提案名称 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="案件号:">
            <el-input v-model="caseInfo.caseNumber" disabled readonly></el-input><!-- 系统内部案件编号 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="申请号:">
            <el-input v-model="caseInfo.applicationNumber" disabled readonly></el-input><!-- applicationNumber: 申请号或专利号-数据库取 -->
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="案件状态:">
            <el-input v-model="caseInfo.caseStatus" disabled readonly></el-input><!-- 当前案件状态 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="案件名称:">
            <el-input v-model="caseInfo.caseName" disabled readonly></el-input><!-- nameInvention: 发明创造名称-数据库取 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="申请类型:">
            <el-select v-model="caseInfo.applicationType" placeholder="请选择" disabled>
              ...
              <el-option label="外观(2)" value="2"></el-option>
            </el-select><!-- businessType: 业务类型，发明为0，实用为1，外观为2 -->
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="处理事项:">
            <el-input v-model="caseInfo.processItem" disabled readonly></el-input><!-- 当前处理的具体事项 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="代理机构:">
            ...
            </el-select><!-- nameOfOrganization: 被委托专利代理机构名称 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="发文日:">
            <el-input v-model="caseInfo.documentDate" disabled readonly></el-input><!-- date: 签章日期 -->
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="官方期限:">
            <el-input v-model="caseInfo.officialDeadline" disabled readonly></el-input><!-- 官方规定的补正期限 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="客户名称:">
            ...
            </el-select><!-- customerName: 申请人或专利权人姓名或名称-数据库取 -->
          </el-form-item>
        </el-col>
        <el-col :span="8">
    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="请求书" name="request-content">
        ...
        <el-row :gutter="20" style="margin: 10px 0 20px;">
          ...
          <el-col :span="6">
            <el-select v-model="correctionTempData.submission_type" placeholder="请选择" style="width: 100%;">
              <el-option label="请选择" value=""></el-option>
            </el-select><!-- 递交方式选择 -->
          </el-col>
          <el-col :span="6">
            <el-select v-model="correctionTempData.submission_type2" placeholder="请选择" style="width: 100%;">
              <el-option label="请选择" value=""></el-option>
            </el-select><!-- 递交类型细分 -->
          </el-col>
          <el-col :span="6">
            <el-select v-model="correctionTempData.submission_type3" placeholder="请选择" style="width: 100%;">
              <el-option label="请选择" value=""></el-option>
            </el-select><!-- fileType: 文件类型，发明，实用，外观新申请为0，其他为1 -->
          </el-col>
          <el-col :span="4">
            <el-button type="primary" :disabled="!correctionTempData.save_button_enabled || isUploading">保存</el-button>
          ...
          <el-form label-width="auto">
            <el-form-item>
              <el-radio-group v-model="correctionTempData.regulation_50_checked">
                <el-radio :value="true">
                  ...
                  ></el-date-picker><!-- correction_deadline: 补正期限日期 -->
                  <span>发出的</span>
                  <el-input
                    v-model="correctionTempData.issued_document"
                    placeholder="发文类型"
                    style="width: 120px; margin: 0 5px;"
                  ></el-input><!-- issued_document: 发出的文档类型 -->
                  <span>（发文号</span>
                  <el-input
                    v-model="correctionTempData.document_number"
                    placeholder="通知书序号"
                    style="width: 120px; margin: 0 5px;"
                  ></el-input><!-- document_number: 发文号 -->
                  <span>）进行补正</span>
                </el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item>
              <el-radio-group v-model="correctionTempData.regulation_57_checked">
                <el-radio :value="true">
                  根据专利法实施细则第57条的规定,请求对上述专利申请主动提出修改。
                </el-radio><!-- regulation_57_checked: 根据专利法实施细则第57条的规定 -->
              </el-radio-group>
            </el-form-item>
          </el-form>
        </el-card>
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-row style="margin-bottom: 10px;">
            <el-col>
              <el-button type="primary" @click="openAmendmentModal">添加补正内容</el-button><!-- 打开补正内容添加弹窗，用于添加correctionContent -->
            </el-col>
          </el-row>
          <el-table :data="correctionList" border style="width: 100%;">
            <el-table-column prop="sequence" label="序号" width="80"></el-table-column><!-- correctionContent.sequence: 补正项序号 -->
            <el-table-column prop="document_name" label="文件名称" width="150"></el-table-column><!-- correctionContent.document_name: 补正涉及的文件名称 -->
            <el-table-column prop="position_in_document" label="文件中的位置" width="150"></el-table-column><!-- correctionContent.position_in_document: 补正内容在文件中的位置 -->
            <el-table-column prop="correction_before" label="补正前" min-width="200"></el-table-column><!-- correctionContent.correction_before: 补正前的内容 -->
            <el-table-column prop="correction_after" label="补正后" min-width="200"></el-table-column><!-- correctionContent.correction_after: 补正后的内容 -->
            <el-table-column label="操作" width="100">
              <template #default="{ row }">
                <el-button type="danger" size="small" @click="deleteCorrection(row.sequence)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <div v-if="correctionList.length === 0" style="text-align: center; padding: 20px; color: #999;">
            （暂无补正内容）
          </div>
        </el-card>
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-row style="margin-bottom: 10px;">
             <el-col :span="4" >
              <el-select v-model="correctionTempData.revision_mode" placeholder="修订模式" style="width: 100%;">
                <el-option label="修订模式" value="revision_mode" selected></el-option>
                <el-option label="替换模式" value="upload_file"></el-option>
                <el-option label="其他模式" value="other_mode"></el-option>
              </el-select><!-- 修订模式选择下拉框，默认为修订模式 -->
            </el-col>
          ...
          <el-table :data="uploadedFilesList" border style="width: 100%;">
            ...
            <el-table-column prop="original_filename" label="上传文件名称" width="150"></el-table-column>
            ...
            <el-table-column prop="file_description" label="文件简称" width="120"></el-table-column>
            ...
            <el-table-column label="操作" width="100">
              <template #default="{ row }">
                <el-button type="danger" size="small" @click="deleteUploadedFile(row.files_id ?? resolveFilesIdForUploadedRow(row))">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <div v-if="uploadedFilesList.length === 0" style="text-align: center; padding: 20px; color: #999;">
            （填写数据）
          </div>
          <el-row style="margin-top: 10px;">
            ...
            <el-col :span="8">
              <el-input v-model="correctionTempData.filed_document_barcode" placeholder="请输入备案编号"></el-input><!-- filed_document_barcode: 已备案的明文文件条码号 -->
            </el-col>
          </el-row>
        </el-card>
      </el-tab-pane>
      ...
      <el-tab-pane label="待转档文件" name="pending-content">
        <el-card>
          ...
          <el-row style="margin-bottom: 20px;" :gutter="18">
            <el-col :span="2">
              <el-button type="info" @click="submitAdditionsCorrections">启动转档XML</el-button><!-- 启动转档流程，生成XML文件 -->
            </el-col>
          </el-row>
        ...
        <el-table :data="processedFilesList" border style="width: 100%;">
          ...
          <el-table-column label="操作" fixed="right" width="280">
            <template #default="scope">
              <el-button type="primary" size="small" @click="handleProcessedFileView(scope.row)" :disabled="!isPdfFileRow(scope.row)">预览PDF</el-button>
              <el-button type="success" size="small" @click="handleProcessedZipDownload(scope.row)" :disabled="!isZipFileRow(scope.row)">下载ZIP</el-button>
              <el-button type="danger" size="small" @click="deleteProcessedFile(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div v-if="processedFilesList.length === 0" style="text-align: center; padding: 16px; color: #999;">
          （暂无已转档文件）
        </div>
      </el-tab-pane>
  ...
  <el-dialog v-model="showUploadModal" title="上传文件" width="400px">
    <!-- 文件上传区域 -->
    <div style="border: 1px dashed #d9d9d9; border-radius: 4px; padding: 20px; text-align: center; margin-bottom: 15px;">
      <div style="font-size: 48px; color: #d9d9d9; margin-bottom: 10px;">⬆️</div>
      <p style="color: #666; margin-bottom: 10px;">选择DOCX后自动上传</p>
      <input type="file" accept=".docx" @change="handleFileSelect" style="display: none;" ref="fileInputRef" />
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="saveUploadedFile" :disabled="isUploading || !uploadedFile">保存</el-button>
      </span>
    </template>
  </el-dialog>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="文件名称">
            <el-input v-model="amendmentForm.document_name" placeholder="请输入文件名称" /><!-- correctionContent.document_name: 文件名称 -->
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="文件中的位置">
            <el-input v-model="amendmentForm.position_in_document" placeholder="请输入文件位置" /><!-- correctionContent.position_在文件中的位置 -->
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="补正前">
            <el-input type="textarea" v-model="amendmentForm.correction_before" :rows="4" placeholder="请输入补正前内容" /><!-- correctionContent.correction_before: 补正前 -->
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="补正后">
            <el-input type="textarea" v-model="amendmentForm.correction_after" :rows="4" placeholder="请输入补正后内容" /><!-- correctionContent.correction_after: 补正后 -->
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
