  ...
  improperString: JSON.stringify(
    {
      notifications: [
        ...
        { type: 1, date: '2025-10-08', name: '针对第一次审查意见的补充陈述', serialNumber: '20251008002' },
...
const saveForm = async () => {
  try {
    ...
    const cpRaw = String(idQueryForm.caseProcessesId || urlParams.caseProcessesId || caseProcessesIdFixed.value || '').trim()
    ...
    const payload: any = {
      ...
      docx: (improperForm.opinionContent || '').trim() || statementFile.value?.name || improperForm.docx || 'improper_statement.docx'
    }
...
const refreshFilesFromServer = async (retryCount = 0, mergeMode = false) => {
  try {
    ...
    const { opinions, comparisons, proofs, opinionSpecialUrls: opSpecUrls, comparisonSpecialUrls: cmpSpecUrls } = await fetchOpinionFilesBySubmissionLocal({
    ...
    if (mergeMode) {
      ...
      if (opinions.length > 0) {
        ...
        opinions.forEach(newFile => {
          ...
          if (existingIndex >= 0) {
            opinionFileList.value[existingIndex] = { ...opinionFileList.value[existingIndex], ...newFile }
          }
        })
      }
      ...
      if (comparisons.length > 0) {
        ...
        comparisons.forEach(newFile => {
          const existingIndex = comparisonFileList.value.findIndex(f => f.fileId === newFile.fileId)
          if (existingIndex >= 0) {
            comparisonFileList.value[existingIndex] = { ...comparisonFileList.value[existingIndex], ...newFile }
          }
        })
      }
...
const rebuildPendingFiles = () => {
  try {
    ...
    const build = (it: any): PendingItem => {
      const name = String(it?.uploadedFileName || it?.fileName || it?.original_filename || it?.name || '')
    ...
    const combined = [...(opinionFileList.value || []), ...(comparisonFileList.value || [])].map(build)
...
watch([opinionFileList, comparisonFileList], () => {
  rebuildPendingFiles()
}, { deep: true })
...
const fetchOpinionFilesBySubmissionLocal = async (params: OpinionQueryParams) => {
  ...
  const rawList = Array.isArray((res as any)?.data)
    ? (res as any).data
    : (Array.isArray(res) ? res : ((res as any)?.files || (res as any)?.fileList || []))
  ...
  for (const it of rawList) {
    ...
    const name = String(it?.fileName || it?.realName || it?.filename || it?.originalName || it?.name || '未命名')
    ...
    const opinionBase = (
      internalCode === 'B100012' ||
      /意见陈述|陈述/i.test(fileType) ||
  ...
  return { opinions, comparisons, proofs, raw: rawList, opinionSpecialUrls: opinionSpecialUrlsLocal, comparisonSpecialUrls: comparisonSpecialUrlsLocal }
}
...
const submitForm = async () => {
  ...
  if (comparisonFile.value && comparisonFile.value.type !== 'application/pdf' && !/\.pdf$/i.test(comparisonFile.value.name || '')) {
  ...
  try {
    ...
    const statementRef = latestOpinionItem?.fileBaseUrl || latestOpinionItem?.fileUrl || (statementFile.value?.name || '空陈述文件.docx')
    formData.append('Statement', statementRef)

    const latestComparisonItem: any = pickLatestSpecialOne(comparisonFileList.value)
    const comparisonRef = latestComparisonItem?.fileBaseUrl || latestComparisonItem?.fileUrl || (comparisonFile.value?.name || '空对比页.pdf')
    formData.append('comparisonPage', comparisonRef)

    // 字符串字段 - docx：优先发送用户填写的意见文本，其次才用文件名
    const docxValue = (improperForm.opinionContent || '').trim() || (statementFile.value?.name || 'improper_statement.docx')
    ...
    if (improperForm.improperString && improperForm.improperString !== '{}') {
      ...
      const statementJson = {
        notifications: notificationsPayload.length ? notificationsPayload : improperForm.notifications,
        RegulationAmendment: true
      }
    ...
    console.log('文件信息:', {
      statementFile: statementFile.value ? { name: statementFile.value.name, type: statementFile.value.type, size: statementFile.value.size } : null,
      comparisonFile: comparisonFile.value ? { name: comparisonFile.value.name, type: comparisonFile.value.type, size: comparisonFile.value.size } : null,
      statementRef,
      comparisonRef,
      docxValue
    })
...
const addOpinionStatement = () => {
  ...
  uploadAccept.value = '.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document'
  showUploadModal.value = true
}
  ...
  if (uploadTarget.value === 'opinion' && uploadFile?.name) {
    ...
    if (uploadResult && (uploadResult as any).success) {
      ...
      if (lastFileIndex >= 0) {
        ...
        const baseUrl = uploadData?.base_url || uploadData?.url || uploadData?.file_url || uploadData?.fileUrl || ''
        if (baseUrl) {
      ...
      const errorMsg = (uploadResult as any)?.error || (uploadResult as any)?.message || '未知错误'
...
function applyOpinionStatementToForm(data: any) {
  try {
    ...
    // 深层页面与上传文档
    const deepDocs = Array.isArray(data?.deepPageDocuments?.documents) ? data.deepPageDocuments.documents : []
    const uploadedDocs = Array.isArray(data?.uploadedDocuments?.documents) ? data.uploadedDocuments.documents : []
...
const downloadFile = async (row: any) => {
  try {
    ...
    // 提取文件名（尽量使用原始名称）
    let fileName = row.attachmentName || row.fileName || row.uploadFileName || row.upload_filename || '下载文件'
...
const viewFile = async (row: any) => {
  ...
  const isZip = String(fileName).toLowerCase().endsWith('.zip') || /\.zip(\?|$)/i.test(String(fileUrl))
  if (isZip) {
    try {
      const resp = await fetch(fileUrl, { method: 'GET' })
      if (!resp.ok) {
...
<main>
        ...
        <el-col :span="6">
          <el-form-item label="案件状态:">
            <el-select v-model="improperForm.caseStatus" placeholder="后台配置选择" disabled>
              ...
              <el-option
                v-if="improperForm.caseStatus && !['pending','processing','completed'].includes(String(improperForm.caseStatus))"
        ...
        <el-col :span="6">
          <el-form-item label="代理机构:">
            <el-select v-model="improperForm.agency" placeholder="后台配置选择" disabled>
              ...
              <el-option
                v-if="improperForm.agency && !['agency_a','agency_b'].includes(String(improperForm.agency))"
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="发文日:">
            <el-date-picker v-model="improperForm.issueDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" disabled/>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="官方期限:">
            <el-date-picker v-model="improperForm.officialDeadline" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" disabled/>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="内部期限:">
            <el-date-picker v-model="improperForm.internalDeadline" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" disabled/>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="业务人员:">
            <el-select v-model="improperForm.businessPersonnel" placeholder="后台配置选择" disabled>
              <el-option label="张三" value="zhangsan" />
              <el-option label="李四" value="lisi" />
              <el-option
                v-if="improperForm.businessPersonnel && !['zhangsan','lisi'].includes(String(improperForm.businessPersonnel))"
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="案件ID:">
            <el-input v-model="caseIdFixed" disabled readonly style="background-color: #f5f7fa; color: #909399;"/>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="处理事项ID:">
            <el-input v-model="caseProcessesIdFixed"  disabled readonly style="background-color: #f5f7fa; color: #909399;"/>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <!-- 空列占位 -->
    ...
    <el-tab-pane label="请求书" name="request">
      ...
      <div class="tab-content">
        ...
        <el-row :gutter="20" style="margin-bottom: 20px;">
          ...
          <el-col :span="4">
            <el-select v-model="improperForm.submissionType" placeholder="后台配置选择" style="width: 100%;">
              <el-option label="电子申请" value="electronic" />
              <el-option label="纸质申请" value="paper" />
            </el-select>
          </el-col>
          <el-col :span="4">
            <el-select v-model="improperForm.submissionType" placeholder="后台配置选择" style="width: 100%;" disabled>
              <el-option label="普通递交" value="normal" />
              <el-option label="加急递交" value="urgent" />
            </el-select>
          </el-col>
          <el-col :span="4">
            <el-select v-model="improperForm.submissionType" placeholder="递交类型" style="width: 100%;" disabled>
        ...
        <el-card style="margin-bottom: 20px;">
          <template #header>
            ...
            <el-radio-group v-model="improperForm.requestType" style="display: block;">
              ...
              <div style="margin-bottom: 15px; display: block;">
                <el-radio label="supplementary_notice_response" style="display: block; margin-bottom: 5px;">
                  针对国家知识产权局于
                  <el-date-picker
            ...
            <el-button type="success" @click="addOpinionStatement" style="margin-left: 10px;" :disabled="(improperForm.opinionContent || '').trim().length > 0">
              添加意见陈述word
            </el-button>
          ...
          <el-table :data="processedFiles" style="width: 100%;">
            ...
            <el-table-column label="操作" width="150">
              <template #default="scope">
                ...
                <el-link type="primary" @click="viewFile(scope.row)" style="margin-left: 10px;">查看</el-link>
              </template>
            </el-table-column>
          </el-table>
  ...
  >
    <el-table :data="allDataList" height="500" style="width: 100%">
      ...
      <el-table-column prop="proofFileRecordNumber" label="备案编号" width="150"></el-table-column>
