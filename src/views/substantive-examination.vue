...
const submitDelete = () => {
  ...
  const deleteUrl = `http://106.52.57.109:8082/api/substantive-examination/delete?case_processes_id=${deleteForm.case_processes_id}&case_id=${deleteForm.case_id}`;
...
const saveExaminationRequest = async () => {
  try {
    ...
    // 构建请求URL - 使用固定的ID值
    const baseUrl = 'http://106.52.57.109:8082/api/substantive-examination/save'
    const urlParams = getParamsFromUrl();
...
const queryById = async (processId?: string, caseId?: string) => {
  try {
    ...
    // 构建查询URL
    const url = `http://106.52.57.109:8082/api/substantive-examination/by-case?case_processes_id=${encodeURIComponent(case_processes_id)}&case_id=${encodeURIComponent(case_id)}`
    ...
    if (result.success) {
      ...
      if (result.data) {
        ...
        caseInfo.value = {
          ...
          applicationType: result.data.submissionType.includes('实用') ? '实用新型' : result.data.submissionType.includes('外观') ? '外观设计' : '发明专利',
        ...
        requestForm.value = {
          ...
          examineMatter: result.data.examineMatter !== undefined ? result.data.examineMatter : requestForm.value.examineMatter,
          deferredExamination: result.data.deferredExamination || requestForm.value.deferredExamination,
          filedProofDocumentNumber: result.data.filedProofDocumentNumber || requestForm.value.filedProofDocumentNumber,
          pctInternationalApplication: result.data.pctInternationalApplication || requestForm.value.pctInternationalApplication,
 ...
 <main>
  ...
  >
    <el-form label-width="120px">
      <el-form-item label="处理事项id:">
        <el-input v-model="idQueryForm.case_processes_id" placeholder="请输入处理事项ID"></el-input>
      </el-form-item>
    ...
    <el-tabs v-model="activeTab" >
      <el-tab-pane label="请求书" name="request-content">
        ...
        <el-row :gutter="20" style="margin-bottom: 30px;">
          ...
          <el-col :span="5">
            <el-select v-model="requestForm.submissionType" placeholder="请选择" style="width: 100%;">
              <el-option label="请选择" value=""></el-option>
            </el-select>
          </el-col>
          <el-col :span="5">
        ...
        <el-card>
          ...
          <el-form :model="requestForm" label-width="auto">
            ...
            <el-form-item>
              <el-select v-model="requestForm.additionalFile" placeholder="请选择附加文件类型" style="width: 200px;" @change="handleAdditionalFileChange">
  ...
  >
    <el-form :model="deleteForm" label-width="100px">
      <el-form-item label="处理事项ID:">
        <el-input v-model="deleteForm.case_processes_id" placeholder="请输入处理事项ID"></el-input>
      </el-form-item>
      <el-form-item label="案件ID:">
