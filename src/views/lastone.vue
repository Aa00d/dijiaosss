...
const saveForm = async () => {
  try {
    ...
    console.log('  - 完整URL验证:', url.includes('submission_page=意见陈述') ? '✅ 包含submission_page参数' : '❌ 缺少submission_page参数')
        ...
        try {
          if (finalCaseProcessesId && finalCaseId) {
            ...
            if (Array.isArray(dataArray) && dataArray.length > 0) {
              ...
              formData.proofFileRecordNumber = data.proofFileRecordNumber || data.proof_file_record_number || ''
              ...
              formData.representationDeclaration = toBoolean(data.delegationStatement || data.delegation_statement)
              formData.requestConversionFee = toBoolean(data.requestConversionFee || data.request_conversion_fee)
              ...
              if (data.feeDetails || data.fee_details) {
                try {
                  ...
                  const feeDetails = typeof feeDetailsStr === 'string' ? JSON.parse(feeDetailsStr) : feeDetailsStr
              ...
              if (data.bankName || data.bank_name || data.bankBranch || data.bank_branch || data.bankAccount || data.bank_account || data.accountHolderName || data.account_holder_name) {
              ...
              if (data.recipientName || data.recipient_name || data.recipientPhone || data.recipient_phone || data.recipientEmail || data.recipient_email) {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (fileList.value.filter(item => item.url).length > 0) {
      console.log('✅ 添加fileNameDtos数组，文件数量:', fileList.value.filter(item => item.url).length)
...
const executeIdQuery = async () => {
  ...
  try {
    ...
    formData.representationDeclaration = data.delegationStatement || data.delegation_statement || false // 受委托人受托请求退款确认

    // 其他请求类型标志 - 兼容两种格式
    formData.requestConversionFee = data.requestConversionFee || data.request_conversion_fee || false
    ...
    if (data.intellectualPropertyRight) {
      try {
        const intellectualPropertyRight = typeof data.intellectualPropertyRight === 'string'
          ? JSON.parse(data.intellectualPropertyRight)
          : data.intellectualPropertyRight
        formData.intellectualPropertyRight = {
          targetDate: intellectualPropertyRight.target_date || intellectualPropertyRight.targetDate || '',
          applicationPatentInfo: intellectualPropertyRight.application_patent_info || intellectualPropertyRight.applicationPatentInfo || '',
          noticeNumber: intellectualPropertyRight.notice_number || intellectualPropertyRight.noticeNumber || '',
        ...
        if (intellectualPropertyRight.target_date || intellectualPropertyRight.targetDate) {
          formData.noticeDate = (intellectualPropertyRight.target_date || intellectualPropertyRight.targetDate).replace(/\//g, '-')
        }
        if (intellectualPropertyRight.notice_number || intellectualPropertyRight.noticeNumber) {
          formData.serialNumber = intellectualPropertyRight.notice_number || intellectualPropertyRight.noticeNumber
        }
        if (intellectualPropertyRight.application_patent_info || intellectualPropertyRight.applicationPatentInfo) {
          formData.noticeName = intellectualPropertyRight.application_patent_info || intellectualPropertyRight.applicationPatentInfo
        }
      } catch (e) {
...
const handlePendingFileSelect = async (event: Event) => {
  ...
  // 使用URL参数、内部保存的值或默认值
  const finalCaseProcessesId = urlCaseProcessesId || currentCaseProcessesId.value || parseInt(DEFAULT_CASE_PROCESSES_ID)
...
const loadDefaultData = async () => {
  try {
    ...
    const jsonData = await fetchOpinionStatementUnifiedByCase(parseInt(caseProcessesId), parseInt(caseId))
    ...
    formData.refundRequestType = data.refundRequestType || data.refund_request_type || 1 // 1-细则111条第3款；2-272号公告
    formData.refundReasonDetail = data.refundReasonDetail || data.refund_reason_detail || '' // 退款请求详细说明
    formData.representationDeclaration = toBoolean(data.delegationStatement || data.delegation_statement) // 受委托人受托请求退款确认
    ...
    if (data.bankName || data.bank_name || data.bankBranch || data.bank_branch || data.bankAccount || data.bank_account || data.accountHolderName || data.account_holder_name) {
      ...
      try {
        ...
        formData.refundAccountBranch = bankAccountInfo.branch_name || bankAccountInfo.bankBranch || ''
        formData.refundAccountNumber = bankAccountInfo.account_number || bankAccountInfo.bankAccount || ''
        formData.refundAccountName = bankAccountInfo.account_holder_name || bankAccountInfo.accountHolderName || ''
      } catch (e) {
        console.error('解析bankAccountInfo失败:', e)
      }
    }
    ...
    if (data.recipientName || data.recipient_name || data.recipientPhone || data.recipient_phone || data.recipientEmail || data.recipient_email) {
      ...
      try {
        const recipientInfo = JSON.parse(data.recipientInfo)
        formData.recipientName = recipientInfo.recipient_name || recipientInfo.recipientName || recipientInfo.name || ''
        formData.recipientPhone = recipientInfo.recipient_phone || recipientInfo.recipientPhone || recipientInfo.phone || ''
        formData.recipientEmail = recipientInfo.recipient_email || recipientInfo.recipientEmail || recipientInfo.email || ''
      } catch (e) {
        console.error('解析recipientInfo失败:', e)
      }
    }
    ...
    if (data.uploaded_files) {
      try {
        const uploadedFiles = typeof data.uploaded_files === 'string' ? JSON.parse(data.uploaded_files) : data.uploaded_files
      ...
      // 兼容旧字段名
      try {
        const uploadedFiles = typeof data.uploadedFiles === 'string' ? JSON.parse(data.uploadedFiles) : data.uploadedFiles
        if (Array.isArray(uploadedFiles)) {
          fileList.value = []
          uploadedFiles.forEach((file: any, index: number) => {
            const fileItem: FileItem = {
...
const fetchUploadedFilesList = async (caseProcessesId: string, caseId: string) => {
  try {
    ...
    const url = `http://8.140.210.30:8082/api/files/list-by-submission?case_processes_id=${caseProcessesId}&case_id=${caseId}&submission_page=意见陈述`
    ...
    if (resultJson && resultJson.success && resultJson.files && Array.isArray(resultJson.files)) {
      ...
      fileList.value = otherFiles.map((file: any, index: number) => ({
        ...
        fileType: file.fileCategoryMinor || file.file_category_minor || file.internalCode || file.internal_code || '',
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="案件名称:" :disabled="true">
              <el-input v-model="formData.caseName" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="申请类型:" :disabled="true">
              <el-select v-model="formData.applicationType" placeholder="后台配置选择" :disabled="true">
                <el-option label="后台配置选择" value=""></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="处理事项:" :disabled="true">
              <el-input v-model="formData.processingMatters" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="发文日:" :disabled="true">
              <el-input v-model="formData.issueDate" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="官方期限:" :disabled="true">
              <el-input v-model="formData.officialDeadline" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="内部期限:" :disabled="true">
              <el-input v-model="formData.internalDeadline" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="业务人员:" :disabled="true">
              <el-select v-model="formData.businessPersonnel" placeholder="后台配置选择" :disabled="true">
                <el-option label="后台配置选择" value=""></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="客户名称:" :disabled="true">
              <el-input v-model="formData.customerName" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="技术主导:" :disabled="true">
              <el-input v-model="formData.technicalLead" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item label="双报案件:" :disabled="true">
              <el-input v-model="formData.dualReportCase" placeholder="单行输入" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
        ...
        <!-- 请求书内容 -->
         <div class="tab-content">
          ...
          <el-row :gutter="20" style="margin-bottom: 20px;">
            ...
            <el-col :span="5">
              <el-select v-model="formData.submissionType" placeholder="后台配置选择" style="width: 100%;">
          ...
          <el-card style="margin-bottom: 20px;">
            ...
            <el-radio-group v-model="formData.requestType" style="display: flex; flex-direction: column; align-items: flex-start; gap: 10px;">
              ...
              <!-- 退款请求子选项 -->
              <div v-if="formData.requestType === 'refund'" style="margin-left: 30px; margin-top: 10px; margin-bottom: 10px;">
                <el-radio-group v-model="formData.refundSubtype" style="display: flex; flex-direction: column; align-items: flex-start; gap: 10px;">
                  <el-radio value="patent_law_111">根据专利法实施细则第111条第3款的规定请求退款 <span style="color: #909399; font-size: 12px;">(提示:票据号码(收据号)请填写票据右上角号码)</span></el-radio>
                                  <!-- 专利法选项显示表格 -->
                <el-card v-if="formData.refundSubtype === 'patent_law_111'" style="margin-top: 10px;width: 70rem;">
                  <template #header>
                    <span style="font-size: 14px;">退款请求详情</span>
                  </template>
                  <el-table :data="formData.feeItems || []" style="width: 100%">
                    <el-table-column prop="seq" label="序号" width="80" align="center"></el-table-column>
                    <el-table-column prop="feeType" label="费用类型" width="200"></el-table-column>
                  ...
                  <div v-if="!formData.feeItems || formData.feeItems.length === 0" style="text-align: center; padding: 20px; color: #909399;">
                    暂无退款请求详情数据
                  </div>
                </el-card>
                  <el-radio value="ip_announcement_272">根据三家知识产权局第272号公告请求退50%实审费 <span style="color: #909399; font-size: 12px;">(提示:票据号码(收据号)请填写票据右上角号码)</span></el-radio>
                </el-radio-group>



                <!-- 知识产权局公告选项显示票据号码输入框 -->
                <el-card v-if="formData.refundSubtype === 'ip_announcement_272'" style="margin-top: 10px;">
                  <template #header>
              ...
              <!-- 退款账户表单，仅在选择退款请求时显示 -->
              <el-card v-if="formData.requestType === 'refund'" style="margin-top: 15px; margin-left: 30px;">
                <template #header>
              ...
              <el-card v-if="formData.requestType === 'refund'" style="margin-top: 15px; margin-left: 30px;">
                ...
                <el-radio-group v-model="formData.authorizationDeclaration">
                  <el-radio :value="true">
                    受缴款人委托请求退款
                    <span style="color: #909399; font-size: 12px;">(提示:此处所称"缴款人"为收费票据"交款人"栏目填写的姓名或者名称)</span>
                  </el-radio>
                </el-radio-group>
              </el-card>
              ...
              <div>
                <el-radio value="no_receipt">缴纳专利费用后未收到国家知识产权局开出的费用收据</el-radio>
              </div>
              <div>
              ...
              <div v-if="formData.requestType === 'convert'" style="margin-top: 15px; margin-bottom: 20px;">
                <el-card >
              ...
              <el-col :span="8">
                <el-form-item label="上传附加文件:">
                  ...
                  >
                    ...
                    <el-option value="bank-copy-stamped" label="加盖银行公章或经公证的银行汇单复印件"></el-option>
                    <el-option value="post-copy-stamped" label="加盖邮局公章或经公证的邮局汇单复印件"></el-option>
    ...
    >
      <el-form :model="fileUploadForm" label-width="100px">
        ...
        <el-form-item label="显示名称:">
          <el-input
            v-model="fileUploadForm.displayName"
            placeholder="请输入显示名称（可选）"
          />
        </el-form-item>
        <el-form-item label="文件简称:">
          <el-input
            v-model="fileUploadForm.shortName"
            placeholder="请输入文件简称（可选）"
          />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="executeIdQuery" :loading="isLoading" :disabled="isLoading">
            {{ isLoading ? '查询中...' : '查询' }}
          </el-button>
        </span>
      </template>
