      ...
      <el-tabs v-model="activeTab" @tab-change="switchTab">
        <el-tab-pane label="请求书" name="request-content">
          ...
          <div class="tab-content">
              ...
              <el-row :gutter="20">
                <el-col :span="6">
                  <el-form-item label="递交类型:">
                    <el-select
                      v-model="reexaminationData.submissionType"
                      placeholder="后台配置选择"
                    >
                      <el-option label="后台配置选择" value=""></el-option>
                ...
                <el-form-item>
                  <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap">
                    <el-checkbox v-model="reexaminationData.relatedPatentDeclaration">
                      <span
                        :class="
                          reexaminationData.relatedPatentDeclaration
                            ? 'inline-label--active'
                            : 'inline-label--inactive'
                        "
                      >
                        根据 专利法第41条第1款 及 专利法实施细则第60条第1款
                        的规定，对国家知识产权局于
                      </span>
                    </el-checkbox>
                    <el-date-picker
            ...
            <!-- 添加复审意见陈述 -->
            <div class="section">
              <el-button type="primary" @click="addOpinionStatement"
                >添加复审意见陈述word</el-button
              >
              ...
              <el-table :data="opinionStatements" style="width: 100%; margin-top: 10px">
                ...
                <el-table-column label="操作" width="100">
                  <template #default="{ row }">
                    <el-button type="danger" size="small" @click="deleteOpinionStatement(row)"
                      >删除</el-button
                    >
                  </template>
                </el-table-column>
            ...
            <div class="section">
                  ...
                  <el-row :gutter="20" align="middle">
                    <el-col :span="6">
                      <el-form-item label="附件类型:">
                        <el-select
                          v-model="reexaminationData.attachmentType"
                          placeholder="请选择"
                          style="width: 100%"
                    ...
                    <el-dialog v-model="attachmentDialogVisible" title="上传附件" width="500px">
                      ...
                      <template #footer>
                        <span class="dialog-footer">
                          ...
                          <el-button type="primary" @click="confirmAttachmentUpload"
                            >确定</el-button
                          >
                        </span>
                      </template>
              ...
              <el-table :data="evidenceFiles" style="width: 100%">
                ...
                <el-table-column label="操作" width="100">
                  <template #default="{ row }">
                    <el-button type="danger" size="small" @click="deleteEvidenceFile(row)"
                      >删除</el-button
                    >
                  </template>
                </el-table-column>
            ...
            <div class="section">
              ...
              <el-form :model="cpcOptions" label-width="auto">
                <el-row :gutter="20">
                  ...
                  <el-col :span="4">
                    <el-checkbox v-model="cpcOptions.restoreRequest"
                      >复审程序恢复权利请求</el-checkbox
                    >
                  </el-col>
                  <el-col :span="4">
                  ...
                  <el-col :span="4">
                    <el-checkbox v-model="cpcOptions.qualificationProof"
                      >添加主体资格证明</el-checkbox
                    >
                  </el-col>
                  <el-col :span="4">
...
const UPLOAD_BY_BYTES_API_URL = import.meta.env.VITE_API_BASE_URL + "/files/upload-by-bytes";
...
const saveReexaminationRequest = async (data: ReexaminationRequestData) => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/reexamination-request/save?${queryParams.toString()}`,
      {
        method: "POST",
        headers: {
...
const deleteOpinionStatement = async (row: any) => {
  try {
    ...
    // 有id，调用后端删除接口
    await ElMessageBox.confirm(`确认删除文件 "${row.fileName}" 吗？`, "删除确认", {
...
const previewFile = async (row: any) => {
  ...
  const fileName = resolveFileName(row);
  const isPdf = /\.pdf($|\?)/i.test(url) || /\.pdf$/i.test(fileName) || isPdfFile({ fileName });
...
async function uploadOneAttachment(file: File) {
  ...
  if (!internalCode) {
    ElMessage.error(
      `文件类型"${reexaminationData.attachmentType}"没有对应的内部代码，请选择其他文件类型`,
    );
...
const confirmAttachmentUpload = async () => {
  ...
  if (result) {
    ...
    if (rawFile) {
      ...
      try {
        const last =
          Array.isArray(evidenceFiles.value) && evidenceFiles.value.length > 0
            ? evidenceFiles.value[evidenceFiles.value.length - 1]
            : null;
  ...
  if (result) {
    ...
    try {
      ...
      if (uploadedUrl) {
        ...
        const first =
          Array.isArray(opinionStatements.value) && opinionStatements.value.length > 0
            ? (opinionStatements.value[0] as any)
            : null;
...
const loadReexaminationData = async (case_processes_id: string, case_id: string) => {
  try {
    ...
    const data =
      Array.isArray(responseData) && responseData.length > 0 ? responseData[0] : responseData;
...
const submitRecheckRequest = async () => {
  try {
    ...
    const latestAttachment =
      proveAndEvidenceFiles.value.length > 0
