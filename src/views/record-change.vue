<template>
  <div class="record-change">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item label="处理事项:">
              <el-input v-model="caseInfo.processingMatters" placeholder="单行输入" disabled></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
      ...
      <el-tabs v-model="activeTab" @tab-click="handleTabClick">
        <el-tab-pane label="请求书" name="request-content">
          ...
          <el-table :data="changeTableData" class="change-table">
            ...
            <el-table-column prop="changeItemNumber" label="变更项目序号" width="240"></el-table-column>
            <el-table-column prop="applicantChangeType" label="申请人变更类型" width="240"></el-table-column>
            <el-table-column prop="specificChangeContent" label="具体变更内容" width="230"></el-table-column>
            ...
            <el-table-column label="操作" width="220">
              <template #default="scope">
                <el-button size="small" type="primary" @click="editRow(scope.$index)">编辑</el-button>
                <el-button size="small" type="primary" @click="deleteRow(scope.$index)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <div class="add-attachment-section">
            ...
            <el-dialog v-model="pdfDialogVisible" title="上传附件" width="1200px">
                ...
                <table class="attachment-dialog-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>
                        <el-select v-model="attachmentForm.change_info" placeholder="请选择" style="width: 100%;">
                    ...
                    <tr>
                      ...
                      <td>
                        <el-select v-model="attachmentForm.changeReason" placeholder="请选择" style="width: 100%;">
                          <el-option v-for="opt in changeReasonOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
                        </el-select>
                      </td>
                      <td>
                        <div class="inline-field">
                          <span class="label-inline">文件说明</span>
                          <el-input v-model="attachmentForm.attachmentSource" placeholder="例如：后台管理提交"></el-input>
                        </div>
                      </td>
                    </tr>
                      ...
                      <td colspan="3">
                         <div v-if="isApplicantRename">
                           <div class="scenario-actions">
                             <el-button size="small" type="primary" @click="addRenameRow">添加一行</el-button>
                           </div>
                           <el-table :data="renameRows" border style="width: 100%;">
                             ...
                             <el-table-column label="变更前顺序" width="160">
                               <template #default="scope"><el-input v-model="scope.row.beforeOrder" /></template>
                             </el-table-column>
                             <el-table-column label="变更前名称" width="220">
                               <template #default="scope"><el-input v-model="scope.row.beforeName" /></template>
                             </el-table-column>
                             <el-table-column label="变更后顺序" width="160">
                               <template #default="scope"><el-input v-model="scope.row.afterOrder" /></template>
                             </el-table-column>
                             <el-table-column label="变更后名称" width="220">
                               <template #default="scope"><el-input v-model="scope.row.afterName" /></template>
                             </el-table-column>
                             <el-table-column label="操作" width="140">
                               <template #default="scope">
                                 <el-button size="small" type="danger" @click="removeRenameRow(scope.$index)">删除</el-button>
                               </template>
                             </el-table-column>
                           </el-table>
                         </div>
                         <div v-else-if="isApplicantTransfer">
                           <div class="scenario-actions">
                             <el-button size="small" type="primary" @click="addTransferRow">添加一行</el-button>
                           </div>
                           <el-table :data="transferRows" border style="width: 100%;">
                             ...
                             <el-table-column label="操作" width="140">
                               <template #default="scope">
                                 <el-button size="small" type="danger" @click="removeTransferRow(scope.$index)">删除</el-button>
                               </template>
                             </el-table-column>
                           </el-table>
                         </div>
                         <div v-else-if="isAgencyTermination">
                           <div class="scenario-actions">
                             <el-button size="small" type="primary" @click="addAgencyTerminationRow">添加一行</el-button>
                           </div>
                           <el-table :data="agencyTerminationRows" border style="width: 100%;">
                             <el-table-column prop="serialNumber" label="序号" width="100" />
                             <el-table-column label="权利人名称" width="220">
                               <template #default="scope"><el-input v-model="scope.row.patenteeName" /></template>
                             </el-table-column>
                             <el-table-column label="变更前代理机构名称" width="240">
                               <template #default="scope"><el-input v-model="scope.row.agencyNameBefore" /></template>
                             </el-table-column>
                             <el-table-column label="机构代码" width="180">
                               <template #default="scope"><el-input v-model="scope.row.agencyCode" /></template>
                             </el-table-column>
                             <el-table-column label="操作" width="140">
                               <template #default="scope">
                                 <el-button size="small" type="danger" @click="removeAgencyTerminationRow(scope.$index)">删除</el-button>
                               </template>
                             </el-table-column>
                           </el-table>
                         </div>
                    ...
                    <tr>
                      ...
                      <td colspan="2">
                        <el-input v-model="attachmentForm.attachmentDescription" placeholder="简要说明"></el-input>
                      </td>
                    </tr>
                    <tr>
                      <td class="label">选择文件</td>
                      <td>
                        <el-button type="primary" @click="triggerPdfUpload">选择 PDF 文件</el-button>
                      </td>
                      <td>
              ...
              <template #footer>
                ...
                <el-button type="primary" :disabled="!pdfFile" @click="confirmPdfDialog">确定</el-button>
              </template>
            </el-dialog>
            <input ref="correctionsInputRef" type="file" accept="image/jpeg,image/jpg" multiple style="display:none" @change="handleCorrectionsSelected" />
          </div>
          ...
          <el-table :data="attachmentTableData" class="attachment-table" style="margin-top: 20px;;">
            ...
            <el-table-column prop="attachmentDescription" label="附件说明" width="240"></el-table-column>
            ...
            <el-table-column label="操作" width="220">
              <template #default="scope">
                <el-button size="small" type="primary" @click="deleteAttachment(scope.$index)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <div class="client-section">
            ...
            <div class="client-options">
              ...
              <el-input v-model="clientInfo.generalPowerNumber" class="power-number-input"></el-input>
            </div>
            ...
            <div class="client-form">
              ...
              <div class="client-form-row">
                <label>委托日期:</label>
                <el-date-picker v-model="clientInfo.entrustmentDate" type="date" class="date-picker" placeholder="yyyy/mm/dd"></el-date-picker>
                <el-button type="primary" @click="triggerCorrectionsUpload">上传委托书</el-button>
                <input ref="pdfInputRef" type="file" accept="application/pdf" style="display:none" @change="handlePdfSelected" />
              ...
              <div v-if="uploadedFilesList.length > 0" class="uploaded-files-preview">
                ...
                <div class="preview-list">
                  <div
                    v-for="(file, index) in uploadedFilesList"
                    :key="index"
                    class="preview-item"
                  >
        ...
        <el-tab-pane label="待转档文件" name="pending-content">
          <div class="btn-group">
            <el-button type="primary" @click="submitAlterationXmlCorrections">启动转档XML</el-button>
          </div>

          <el-table :data="pendingFiles" class="file-table">
            <el-table-column prop="serialNumber" label="序号" width="80"></el-table-column>
            <el-table-column prop="uploadFileName" label="上传文件名称" width="200"></el-table-column>
            ...
            <el-table-column label="操作" width="150">
              <template #default="scope">
                ...
                <el-link type="danger" @click="deleteFile(scope.$index)" style="margin-left: 10px;">删除</el-link>
              </template>
            </el-table-column>
          </el-table>
        </el-tab-pane>
    ...
    <!-- 申请人变更弹窗 -->
    <el-dialog v-model="applicantChangeModalVisible" title="申请人变更" width="80%" :before-close="closeApplicantChangeModal">
      <div style="margin-bottom: 20px;">
        <div class="form-row" style="margin-bottom: 15px;">
          <label style="width: 100px;">申请人变更类型：</label>
          <el-select v-model="applicantChangeData.changeType" style="width: 200px; margin-right: 20px;">
          ...
          <el-input v-model="applicantChangeData.requestDetails" style="width: 200px;" placeholder="单行输入文本"></el-input>
        </div>
          ...
          <table style="width:100%; border-collapse: collapse;">
            ...
            <tbody>
              <tr>
                <td>是否代表人：</td>
                <td>
                  <el-select v-model="applicantChangeData.isRepresentativeBefore" style="width: 200px;">
                ...
                <td>
                  <el-select v-model="applicantChangeData.isRepresentativeAfter" style="width: 200px;">
                    <el-option label="是或者 默认为空" value=""></el-option>
                    <el-option label="是" value="是"></el-option>
                    <el-option label="否" value="否"></el-option>
                  </el-select>
              ...
              <tr>
                <td>姓名或名称：</td>
                <td>
                  <el-input v-model="applicantChangeData.nameBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.nameAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>证件号：</td>
                <td>
                  <el-input v-model="applicantChangeData.idNumberBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.idNumberAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>地址：</td>
                <td>
                  <el-input v-model="applicantChangeData.addressBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.addressAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>邮编：</td>
                <td>
                  <el-input v-model="applicantChangeData.postalCodeBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.postalCodeAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>英文名称：</td>
                <td>
                  <el-input v-model="applicantChangeData.englishNameBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.englishNameAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>英文地址：</td>
                <td>
                  <el-input v-model="applicantChangeData.englishAddressBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.englishAddressAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>电子邮箱：</td>
                <td>
                  <el-input v-model="applicantChangeData.emailBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.emailAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              ...
              <tr>
                <td>电话：</td>
                <td>
                  <el-input v-model="applicantChangeData.phoneBefore" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
                <td>
                  <el-input v-model="applicantChangeData.phoneAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <!-- 代理机构变更弹窗 -->
    <el-dialog v-model="agencyChangeModalVisible" title="变更代理机构" width="600px" :before-close="closeAgencyChangeModal">
      <div style="margin-bottom: 20px;">
        ...
        <div style="display: flex; align-items: center; margin-bottom: 15px;">
          ...
          <el-input v-model="agencyChangeData.agencyBefore" style="width: 200px; margin-right: 15px;" placeholder="单行输入" readonly></el-input>
          <el-input v-model="agencyChangeData.agencyAfter" style="width: 200px;" placeholder="单行输入"></el-input>
        </div>

        <div style="display: flex; align-items: center;">
          <div style="width: 150px;">机构代码</div>
          <el-input v-model="agencyChangeData.agencyCodeBefore" style="width: 200px; margin-right: 15px;" placeholder="单行输入" readonly></el-input>
          <el-input v-model="agencyChangeData.agencyCodeAfter" style="width: 200px;" placeholder="单行输入"></el-input>
        </div>
      </div>
      <template #footer>
        <span class="dialog-footer">
    ...
    <!-- 代理师变更弹窗 -->
    <el-dialog v-model="agentChangeModalVisible" title="变更代理师" width="700px" :before-close="closeAgentChangeModal">
      <div style="margin-bottom: 20px;">
        <div style="display: flex; margin-bottom: 20px;">
          <div style="width: 150px;">变更条目</div>
          <div style="width: 200px;">变更前</div>
            ...
            <tbody>
              <tr>
                ...
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.firstAgentNameBefore" style="width: 200px;" placeholder="单行输入" readonly></el-input>
                </td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.firstAgentNameAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              <tr>
                <td style="width: 150px;">证号</td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.firstAgentCertBefore" style="width: 200px;" placeholder="单行输入" readonly></el-input>
                </td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.firstAgentCertAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              <tr>
                <td style="width: 150px;">电话</td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.firstAgentPhoneBefore" style="width: 200px;" placeholder="单行输入" readonly></el-input>
                </td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.firstAgentPhoneAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table style="width:100%; border-collapse: collapse;">
            <tbody>
              <tr>
                <td style="width: 150px;">姓名</td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.secondAgentNameBefore" style="width: 200px;" placeholder="单行输入" readonly></el-input>
                </td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.secondAgentNameAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              <tr>
                <td style="width: 150px;">证号</td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.secondAgentCertBefore" style="width: 200px;" placeholder="单行输入" readonly></el-input>
                </td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.secondAgentCertAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
              <tr>
                <td style="width: 150px;">电话</td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.secondAgentPhoneBefore" style="width: 200px;" placeholder="单行输入" readonly></el-input>
                </td>
                <td style="width: 200px;">
                  <el-input v-model="agentChangeData.secondAgentPhoneAfter" style="width: 200px;" placeholder="单行输入"></el-input>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div style="display: flex; flex-wrap: wrap;">
        ...
        <div style="width: 50%; padding-right: 20px; box-sizing: border-box;">
          <div style="margin-bottom: 15px;">
            <label style="display: inline-block; width: 120px; text-align: right; margin-right: 10px;">变更项目:</label>
            <el-input v-model="editData.changeItem" style="width: 200px;" readonly></el-input>
          </div>

          <div style="margin-bottom: 15px;">
            <label style="display: inline-block; width: 120px; text-align: right; margin-right: 10px;">请求书中排序:</label>
            <el-input v-model="editData.sortOrder" style="width: 200px;" placeholder="单行输入文本"></el-input>
          </div>

          <div style="margin-bottom: 15px;">
            <label style="display: inline-block; width: 120px; text-align: right; margin-right: 10px;">变更前:</label>
            <el-input v-model="editData.before" style="width: 200px;" placeholder="单行输入"></el-input>
          </div>
        </div>
        ...
        <div style="width: 50%; padding-left: 20px; box-sizing: border-box;">
          <div style="margin-bottom: 15px;">
            <label style="display: inline-block; width: 120px; text-align: right; margin-right: 10px;">具体变更内容:</label>
            <el-input v-model="editData.changeContent" style="width: 200px;" readonly></el-input>
          </div>

          <div style="margin-bottom: 15px;">
            <label style="display: inline-block; width: 120px; text-align: right; margin-right: 10px;">申请人变更类型:</label>
          ...
          <div style="margin-bottom: 15px;">
            <label style="display: inline-block; width: 120px; text-align: right; margin-right: 10px;">变更后:</label>
            <el-input v-model="editData.after" style="width: 200px;" placeholder="单行输入"></el-input>
          </div>
        </div>
      </div>
      <template #footer>
...
const saveBibliographicChange = async (data: BibliographicChangeData, caseProcessesId?: number, caseId?: number) => {
  try {
    ...
    console.log('  - 成功后: 在 case_process_submissions 表记录 (case_processes_id, case_id, submission_page)')
    console.log('')
    console.log('🔧 实际请求信息:')
    console.log('  - URL:', url)
    ...
    console.log('  - 完整URL验证:', url.includes('submission_page=著录变更') ? '✅ 包含submission_page参数' : '❌ 缺少submission_page参数')
...
const loadBibliographicChangeDataFromRecord = async (data: any) => {
  try {
    if (data?.id) {
      ...
      if (data.change || data.changeReason || data.change_reason || data.attachmentName || data.attachment_name || data.attachmentContent || data.attachment_content || data.attachmentCome || data.attachment_come) {
        ...
        if (data.attachmentContent || data.attachment_content) {
          attachmentForm.attachmentDescription = data.attachmentContent || data.attachment_content || ''
        }
        ...
        if (data.attachmentName || data.attachment_name || data.changeReason || data.change_reason) {
...
const executeIdQuery = async () => {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      // 保存查询参数用于内部逻辑（不更新URL）
      console.log('✅ 查询参数已保存（仅内部使用，不更新URL）:', { case_processes_id: caseProcessesId, case_id: caseId })
...
const executeDeleteConfirm = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      const reloadCaseProcessesId = currentCaseProcessesId.value || parseInt(DEFAULT_CASE_PROCESSES_ID)
...
const saveData = async () => {
  try {
    ...
    console.log('✅ 使用ID参数进行保存:', {
      ...
      caseId: finalCaseId,
      source: urlCaseProcessesId && urlCaseId ? 'URL参数' : (currentCaseProcessesId.value && currentCaseId.value ? '内部保存的值' : '默认值')
    })
    ...
    const saveData = {
      ...
      changeItemsInfo: JSON.stringify(changeTableData.value.map(item => ({
      ...
      change: attachmentTableData.value.length > 0 ? (attachmentTableData.value[0]?.changeType || '') : '',
      changeReason: attachmentTableData.value.length > 0 ? (attachmentTableData.value[0]?.changeReason || '') : '',
      attachmentName: attachmentTableData.value.length > 0 ? (attachmentTableData.value[0]?.attachmentName || '') : '',
      attachmentContent: attachmentTableData.value.length > 0 ? (attachmentTableData.value[0]?.attachmentDescription || '') : '',
      attachmentCome: attachmentTableData.value.length > 0 ? (attachmentTableData.value[0]?.attachmentSource || '') : '',
      ...
      setTimeout(async () => {
        try {
          if (finalCaseProcessesId && finalCaseId) {
            const refreshResult = await fetchBibliographicChangeByCase(finalCaseProcessesId, finalCaseId)
...
const changeTableData = ref([
  { serialNumber: 1, changeItem: '(填写数据)', changeItemNumber: '(填写数据)', applicantChangeType: '(填写数据)', specificChangeContent: '(填写数据)', beforeChange: '(填写数据)', afterChange: '(填写数据)' },
  { serialNumber: 2, changeItem: '(填写数据)', changeItemNumber: '(填写数据)', applicantChangeType: '(填写数据)', specificChangeContent: '(填写数据)', beforeChange: '(填写数据)', afterChange: '(填写数据)' },
  { serialNumber: 3, changeItem: '(填写数据)', changeItemNumber: '(填写数据)', applicantChangeType: '(填写数据)', specificChangeContent: '(填写数据)', beforeChange: '(填写数据)', afterChange: '(填写数据)' }
])
...
const changeItemsInfoRaw = ref<string>('')
    ...
    const attachmentTableData = ref([
      { serialNumber: 1, changeType: '', changeReason: '', attachmentName: '(填写数据)', attachmentDescription: '', attachmentSource: '', uploadFile: '' },
      { serialNumber: 2, changeType: '', changeReason: '', attachmentName: '(填写数据)', attachmentDescription: '', attachmentSource: '', uploadFile: '' },
      { serialNumber: 3, changeType: '', changeReason: '', attachmentName: '(填写数据)', attachmentDescription: '', attachmentSource: '', uploadFile: '' }
    ])
    ...
    // 监听总委托书勾选状态
    watch(() => clientInfo.generalPower, (newVal) => {
      if (newVal) {
      ...
      try {
        ...
        if (uploadedFile) {
          ...
          if (fileId) {
            ...
            const url = `http://8.140.210.30:8082/api/files/${fileId}`
            ...
            if (resultJson && resultJson.success !== false) {
              ...
              // 重新获取文件列表，确保数据同步
              const refreshCaseProcessesId = currentCaseProcessesId.value || parseInt(DEFAULT_CASE_PROCESSES_ID)
                ...
                try {
                  ...
                  if (refreshCaseProcessesId && refreshCaseId) {
                    ...
                    const saveData = {
                      ...
                      attachmentName: hasAttachments ? (attachmentTableData.value[0]?.attachmentName || '') : '',
                      attachmentContent: hasAttachments ? (attachmentTableData.value[0]?.attachmentDescription || '') : '',
                      attachmentCome: hasAttachments ? (attachmentTableData.value[0]?.attachmentSource || '') : ''
                    } as any

                    console.log('  - 保存数据:', JSON.stringify(saveData, null, 2))

                    try {
                      const saveResult = await saveBibliographicChange(saveData, refreshCaseProcessesId, refreshCaseId)
                      if (saveResult.success) {
                        console.log('✅ 删除附件后自动保存成功')
                        // 保存成功后，重新加载数据以确保表单数据同步
                        const refreshResult = await fetchBibliographicChangeByCase(refreshCaseProcessesId, refreshCaseId)
                        if (refreshResult.success && refreshResult.data) {
                          await loadBibliographicChangeDataFromRecord(refreshResult.data)
                        }
                      } else {
...
const isZipFile = (file: any) => {
  ...
  return lowerFileName.endsWith('.zip') || lowerFileName.endsWith('.rar') || lowerFileName.endsWith('.7z')
}
...
const viewFile = async (file: any) => {
  try {
    ...
    console.log('👁️ 查看文件:')
    console.log('  - 文件名:', file.fileName || file.file_name || file.uploadFileName || file.attachmentName || '未知')
    console.log('  - URL:', fileUrl)

    // 检查文件名（支持多个字段）
    const fileName = file.fileName || file.file_name || file.uploadFileName || file.attachmentName || ''
...
const openPdfDialog = () => { pdfDialogVisible.value = true }
const closePdfDialog = () => { pdfDialogVisible.value = false }
...
const isApplicantRename = computed(() => attachmentForm.change_info === '申请人/专利权人变更' && ['企业更名','事业单位或社会团体更名','机关法人更名'].includes(attachmentForm.changeReason))
const isApplicantTransfer = computed(() => attachmentForm.change_info === '申请人/专利权人变更' && attachmentForm.changeReason === '转让')
const isAgencyTermination = computed(() => attachmentForm.change_info === '代理机构变更' && ['解除/辞去委托','变更代理机构'].includes(attachmentForm.changeReason))

// 场景表格行类型
interface RenameRow { serialNumber: number; beforeOrder: string; beforeName: string; afterOrder: string; afterName: string }
interface TransferRow { serialNumber: number; beforeOrder: string; beforeName: string; afterOrder: string; afterName: string }
interface AgencyTerminationRow { serialNumber: number; patenteeName: string; agencyNameBefore: string; agencyCode: string }
...
// 行操作
const addRenameRow = () => { renameRows.value.push({ serialNumber: renameRows.value.length + 1, beforeOrder: '', beforeName: '', afterOrder: '', afterName: '' }) }
const removeRenameRow = (idx: number) => { renameRows.value.splice(idx, 1); renameRows.value.forEach((r, i) => r.serialNumber = i + 1) }
const addTransferRow = () => { transferRows.value.push({ serialNumber: transferRows.value.length + 1, beforeOrder: '', beforeName: '', afterOrder: '', afterName: '' }) }
const removeTransferRow = (idx: number) => { transferRows.value.splice(idx, 1); transferRows.value.forEach((r, i) => r.serialNumber = i + 1) }
const addAgencyTerminationRow = () => { agencyTerminationRows.value.push({ serialNumber: agencyTerminationRows.value.length + 1, patenteeName: '', agencyNameBefore: '', agencyCode: '' }) }
const removeAgencyTerminationRow = (idx: number) => { agencyTerminationRows.value.splice(idx, 1); agencyTerminationRows.value.forEach((r, i) => r.serialNumber = i + 1) }
...
const handleCorrectionsSelected = async (e: Event) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      console.log('  - 必须参数: file, case_processes_id, case_id, submission_page, special, internal_code')
      ...
      console.log('🔧 实际请求信息:')
      console.log('  - URL: http://8.140.210.30:8082/api/files/upload-with-code')
      console.log('  - Method: POST')
      ...
      const response = await fetch('http://8.140.210.30:8082/api/files/upload-with-code', requestOptions)
      ...
      if (resultJson && (resultJson.code === 200 || resultJson.success)) {
        ...
        ElMessage.warning(`委托书 ${i + 1}/${files.length} 上传完成，但响应码为: ${resultJson.code || '未知'}`)
      } else {
      ...
      ElMessage.error(`委托书 ${i + 1}/${files.length} 上传失败: ${file.name} - ${error.message || '未知错误'}`)
    }
  }
...
const getFileThumbnail = (file: any): string => {
  ...
  if (fileUrl) {
    ...
    console.log('getFileThumbnail: 文件扩展名:', extension, '是否图片:', imageExtensions.includes(extension))
...
const getFileName = (file: any): string => {
  return file.fileName || file.file_name || file.name || file.original_name || file.originalName || '未知文件'
}
...
const fetchUploadedFilesList = async (caseProcessesId: string, caseId: string) => {
  try {
    ...
    const url = `http://8.140.210.30:8082/api/files/list-by-submission?case_processes_id=${caseProcessesId}&case_id=${caseId}&submission_page=著录变更`
    ...
    if (resultJson && resultJson.success && resultJson.files && Array.isArray(resultJson.files)) {
      ...
      processedFiles.value = special666Files.map((file: any, index: number) => {
        ...
        let formattedDate = ''
        try {
          formattedDate = file.createTime || file.create_time
        ...
        // 判断是否为ZIP文件（在查询后立即判断）
        const lowerFileName = fileName.toLowerCase()
        const isZip = lowerFileName.endsWith('.zip') || lowerFileName.endsWith('.rar') || lowerFileName.endsWith('.7z')
...
function buildMysqlString() {
  ...
  const firstAttachment = (attachmentTableData.value && attachmentTableData.value[0]) ? (attachmentTableData.value[0] as any) : ({} as any)
...
function buildReasonsProofString() {
  ...
  const documentType = (firstAttachment as any).attachmentSource || (firstAttachment as any).attachmentDescription || '（请在附件表中填写证明文件种类）'
  ...
  const signatureArray = (clientInfo.client || '').split(';').map((s: string) => s.trim()).filter(Boolean)
  const agents = [
...
const uploadZipBytes = async (arrayBuffer: ArrayBuffer) => {
  try {
    ...
    console.log('  - 用途: 接收 ZIP 二进制流，解包出 .pdf 与嵌套 .zip，按文件名解析内部代码，上传到 OSS，并落库 dj_files')
    console.log('  - Content-Type: application/octet-stream')
    console.log('  - Body: 原始 ZIP 二进制流（不是 multipart/form-data）')
    console.log('  - 必须参数: case_processes_id, case_id, submission_page')
    console.log('  - 返回格式: { success: boolean, uploaded_count: number, items: array, skipped: array }')
    console.log('')
    console.log('⚠️ 重要说明:')
    console.log('  - 虽然使用二进制流传输，但后端仍会根据ZIP内的文件名解析内部代码')
    console.log('  - 如果文件名无法解析内部代码，文件会被跳过（这是后端的设计限制）')
    console.log('  - 建议后端改进：添加参数支持指定默认内部代码，或对无法解析的文件使用默认内部代码')
    console.log('')
    ...
    console.log('  - Body大小:', `${arrayBuffer.byteLength} bytes (${(arrayBuffer.byteLength / 1024).toFixed(2)} KB)`)
    console.log('')
    ...
    if (resultJson.skipped && Array.isArray(resultJson.skipped) && resultJson.skipped.length > 0) {
      ...
      console.log('')
      console.log('💡 解决方案建议:')
      console.log('  1. 后端改进：建议后端添加参数支持指定默认内部代码（如 `default_internal_code`）')
...
const submitAlterationXmlCorrections = async () => {
  try {
    ...
    console.log('  - ArrayBuffer大小:', `${arrayBuffer.byteLength} bytes (${(arrayBuffer.byteLength / 1024).toFixed(2)} KB)`)
    console.log('')
    ...
    try {
      ...
      if (uploadResult.success) {
        ...
        if (skippedCount > 0) {
          ...
          // 检查是否有"未解析到内部代码"的文件
          const unparsedFiles = uploadResult.skipped.filter((item: any) => item.reason === '未解析到内部代码')
          if (unparsedFiles.length > 0) {
            ...
            console.warn('  - 建议后端改进：添加参数支持指定默认内部代码，或对无法解析的文件使用默认内部代码')
        ...
        if (skippedCount > 0 && uploadedCount === 0) {
          ...
          if (unparsedFiles.length > 0) {
            ...
            console.error('📝 解决方案：')
            console.error('  1. 【推荐】后端改进：添加参数支持指定默认内部代码（如 `default_internal_code`）')
            console.error('  2. 【推荐】后端改进：对无法解析的文件使用默认内部代码，而不是跳过')
            console.error('  3. 临时方案：修改ZIP文件内的文件名，使其包含可识别的文件类型标识')
            console.error('  4. 临时方案：使用标准文件类型名称（如 "权利要求书.pdf"、"说明书.pdf"）')
            console.error('  5. 前端方案：在上传前重命名ZIP内的文件（需要解压和重新打包，较复杂）')
          } else {
            ElMessage.error(`上传失败：所有文件都被跳过，请查看控制台了解详情`)
          }
        } else {
          ElMessage.warning(`上传完成，但响应success为false。已上传 ${uploadedCount} 个文件，跳过 ${skippedCount} 个文件`)
        }
      }
...
const ensureQueryParams = async () => {
  ...
  const urlHasQueryParams = currentUrl.includes('case_processes_id=') && currentUrl.includes('case_id=')

  // 同时检查route.query中是否有参数（双重检查）
  const queryHasCaseProcessesId = route.query.case_processes_id && String(route.query.case_processes_id).trim() !== ''
