...
const fetchCaseSummaryLocal = async (caseId: string | number) => {
  ...
  try {
    ...
    ;(caseInfo as any).applicationType = data?.application_type ?? data?.applicationType ?? data?.type ?? ''
    ;(caseInfo as any).processItem = data?.process_name ?? data?.processItem ?? data?.process_item ?? ''
    ...
    ;(caseInfo as any).internalDeadline = data?.internal_deadline ?? data?.internalDeadline ?? data?.customer_deadline ?? ''
    ;(caseInfo as any).businessPersonName = data?.business_person_name ?? data?.businessPersonName ?? data?.business_person ?? ''
    ;(caseInfo as any).customerName = data?.customer_name ?? data?.customerName ?? data?.client_name ?? ''
    ;(caseInfo as any).techLead = data?.tech_leader_name ?? data?.techLead ?? data?.tech_lead ?? ''
    ;(caseInfo as any).doubleReport = data?.dual_filing ?? data?.same_day_filing ?? data?.doubleReport ?? ''
    ...
    if (data?.priority_examination === true || data?.priority_examination === 1 || data?.priorityExamination === 1 || data?.priorityExamination === true) flags.push('priority')
...
const updateParsedData = () => {
  ...
  try {
    ...
    requesterName.value = reqInfo.requester_name || reqInfo.name || formData.applicantName || (formData as any).invalidApplicantName || ''
    requesterPhone.value = reqInfo.contact_phone || reqInfo.phone || formData.applicantNamePhone || (formData as any).invalidApplicantPhone || ''
    ...
    requesterPhone.value = formData.applicantNamePhone || (formData as any).invalidApplicantPhone || ''
  ...
  commissionDate.value = formData.commissionDate || (powerOfAttorneyInfo.value as any).commission_date || ''
  agencyAddress.value = formData.agencyAddress || (powerOfAttorneyInfo.value as any).address || ''
  agencyPostal.value = formData.agencyPostal || (powerOfAttorneyInfo.value as any).postal_code || ''

  // 从agentInfo解析代理人信息
  if (agentInfoData.agent_1) {
    agent1Name.value = agentInfoData.agent_1.name || (powerOfAttorneyInfo.value as any).agent_1 || ''
    agent1Phone.value = agentInfoData.agent_1.phone || (powerOfAttorneyInfo.value as any).agent_1_phone || ''
    agent1License.value = agentInfoData.agent_1.license || (powerOfAttorneyInfo.value as any).agent_1_license || ''
    agent1Authority.value = agentInfoData.agent_1.authority || (powerOfAttorneyInfo.value as any).agent_1_authority || ''
  }
  if (agentInfoData.agent_2) {
    agent2Name.value = agentInfoData.agent_2.name || (powerOfAttorneyInfo.value as any).agent_2 || ''
    agent2Phone.value = agentInfoData.agent_2.phone || (powerOfAttorneyInfo.value as any).agent_2_phone || ''
    agent2License.value = agentInfoData.agent_2.license || (powerOfAttorneyInfo.value as any).agent_2_license || ''
    agent2Authority.value = agentInfoData.agent_2.authority || (powerOfAttorneyInfo.value as any).agent_2_authority || ''
  }

  // 更新理由表格数据
  // 如果没有invalidRequestReasons，可以根据opinion内容生成默认理由
  if (invalidRequestReasons.value && (invalidRequestReasons.value as any).reasons && (invalidRequestReasons.value as any).reasons.length > 0) {
  ...
  // 更新附件表格数据
  if (attachmentDocuments.value && (attachmentDocuments.value as any).documents && (attachmentDocuments.value as any).documents.length > 0) {
  ...
  // 更新委托书表格数据
  if (powerOfAttorneyAttachments.value && (powerOfAttorneyAttachments.value as any).documents && (powerOfAttorneyAttachments.value as any).documents.length > 0) {
  ...
  const internetEvidenceData = parseJsonData(formData.internetEvidence || (formData as any).internet_evidence || '{}')
...
const updateJsonData = () => {
  ...
  // 更新委托书附件数据
  formData.powerOfAttorneyAttachments = JSON.stringify({ documents: powerOfAttorneyTableData.value })
...
const onSave = async () => {
  ...
  const response = await fetch(apiUrl, {
    ...
    if (response.ok && result.success) {
      ...
      // 获取详细的错误信息
      const errorMessage = result.message || result.error || result.msg || `HTTP ${response.status} 错误`
...
const loadProcessedFiles = async () => {
  try {
    ...
    const processedFiles = filesData.filter((file: any) => {
      const fileSpecial = Number(String(file.special || file.file_special || file.special_type || '0'))
      const isSpecial666 = fileSpecial === 666
      if (!isSpecial666) {
        console.log(`⏭️ 跳过文件 (special=${fileSpecial}):`, file.fileName || file.file_name || file.original_filename)
      }
      return isSpecial666
    })
    ...
    processedFileTableData.value = (mappedProcessed || [])
      ...
      .map((m: any, idx: number) => ({
        ...
        id: m?.id ?? processedFiles[idx]?.id ?? processedFiles[idx]?.fileId ?? processedFiles[idx]?.uid ?? processedFiles[idx]?._id ?? processedFiles[idx]?.record_id ?? null,
    ...
    if (processedFileTableData.value.length === 0) {
      ...
      if (fbResult) {
        let fbFiles: any[] = []
        if (fbResult.success && fbResult.data && Array.isArray(fbResult.data.files)) fbFiles = fbResult.data.files
        ...
        else if (fbResult.success && fbResult.data && fbResult.data.data && Array.isArray(fbResult.data.data)) fbFiles = fbResult.data.data

        const fbProcessed = fbFiles.filter((file: any) => Number(String(file.special || file.file_special || file.special_type || '0')) === 666)
        ...
        processedFileTableData.value = (mappedFb || [])
          ...
          .map((m: any, idx: number) => ({
            ...
            upload_person: m?.uploader ?? '后台记录',
            upload_time: m?.uploadTime ?? m?.createTime ?? '',
            id: m?.id ?? fbProcessed[idx]?.id ?? fbProcessed[idx]?.fileId ?? fbProcessed[idx]?.uid ?? fbProcessed[idx]?._id ?? fbProcessed[idx]?.record_id ?? null,
  ...
  try {
    ...
    fileTableData.value = (mappedAttachments || [])
      ...
      .map((m: any, idx: number) => ({
        ...
        upload_person: m?.uploader ?? '后台记录',
        upload_time: m?.uploadTime ?? m?.createTime ?? '',
        id: m?.id ?? attachmentFiles[idx]?.id ?? attachmentFiles[idx]?.fileId ?? attachmentFiles[idx]?.uid ?? attachmentFiles[idx]?._id ?? attachmentFiles[idx]?.record_id ?? null,
    ...
    powerOfAttorneyTableData.value = (mappedPower || [])
      ...
      .map((m: any, idx: number) => ({
        ...
        upload_person: m?.uploader ?? '后台记录',
        upload_time: m?.uploadTime ?? m?.createTime ?? '',
        id: m?.id ?? powerFiles[idx]?.id ?? powerFiles[idx]?.fileId ?? powerFiles[idx]?.uid ?? powerFiles[idx]?._id ?? powerFiles[idx]?.record_id ?? null,
...
const downloadProcessedFile = async (fileRecord: any) => {
  try {
    ...
    try {
      ...
      if (originalUrl !== downloadUrl && (originalUrl.startsWith('http://') || originalUrl.startsWith('https://'))) {
      ...
      ElMessage.info(`已尝试下载文件: ${filename}（如果浏览器阻止，可能是CORS限制，请检查网络或联系管理员）`)
    }
  } catch (error: any) {
...
const loadData = async (case_processes_id: string, case_id: string) => {
  try {
    ...
    const url = `http://106.52.57.109:8082/api/review-invalid/by-case?case_processes_id=${encodeURIComponent(case_processes_id)}&case_id=${encodeURIComponent(case_id)}&submission_page=${encodeURIComponent('无效宣告')}`
    ...
    if (result.success && result.data) {
      // 处理返回的是数组的情况，取第一个元素
      const dataToUse = Array.isArray(result.data) && result.data.length > 0 ? result.data[0] : result.data
      // 直接更新表单数据，不重复解析
...
const buildSubmitData = () => {
  ...
  const parseAddress = (address: string) => {
    ...
    return {
      ...
      counties: parts[1] ? parts[1] + (address.includes('市') ? '市' : (address.includes('区') ? '区' : (address.includes('县') ? '县' : ''))) : '',
  ...
  const declareData: any = {
    ...
    proposer: {
      name: requesterName.value || requesterInfoData?.name || requesterInfoData?.requester_name || '',
      identity: requesterInfoData?.identity || requesterInfoData?.certificate_number || requesterInfoData?.id_number || '',
      nationality: requesterInfoData?.nationality || requesterInfoData?.country || '中国',
      naplaceBusinesstionality: requesterInfoData?.naplaceBusinesstionality || requesterInfoData?.residence || requesterAddressParsed.city || '',
      cityName: requesterAddressParsed.city || requesterInfoData?.cityName || requesterInfoData?.city || '',
      countiesCities: requesterAddressParsed.counties || requesterInfoData?.countiesCities || requesterInfoData?.counties || '',
      houseNumber: requesterAddressParsed.houseNumber || requesterInfoData?.houseNumber || requesterInfoData?.street_address || requesterAddress.value || '',
      postalCode: requesterInfoData?.postalCode || requesterInfoData?.postal_code || '',
      phone: requesterPhone.value || requesterInfoData?.phone || requesterInfoData?.contact_phone || '',
    ...
    agentDtos: [
      {
        ...
        phone: agent1Phone.value || '',
        agency: agent1Authority.value === '特殊代理' || agent1Authority.value === '1' || String(agent1Authority.value) === 'true'
      },
      {
        name: agent2Name.value || '',
        sort: 2,
        ...
        phone: agent2Phone.value || '',
        agency: agent2Authority.value === '特殊代理' || agent2Authority.value === '1' || String(agent2Authority.value) === 'true'
      }
    ...
    city: agencyAddressParsed.city || powerOfAttorneyData?.city || powerOfAttorneyData?.province || '',
    counties: agencyAddressParsed.counties || powerOfAttorneyData?.counties || powerOfAttorneyData?.city || '',
    houseNumber: agencyAddressParsed.houseNumber || agencyAddress.value || powerOfAttorneyData?.houseNumber || '',
    entrustData: commissionDate.value || powerOfAttorneyData?.commission_date || powerOfAttorneyData?.entrustData || ''
  }
...
const handleSubmit = async () => {
  try {
    ...
    const url = `http://106.52.57.109:8082/api/withdraw-patent-statement/save?case_processes_id=${encodeURIComponent(String(cpId))}&case_id=${encodeURIComponent(String(cId))}&submission_page=${encodeURIComponent(submissionPage)}`

    // 构建 JSON 请求体
    let declareObj: any = null
    let sqlObj: any = null
    try { declareObj = JSON.parse(submitForm.DeclareString) } catch {}
    try { sqlObj = JSON.parse(submitForm.DeclareSqlString) } catch {}
...
const onStartXmlConversion = async () => {
  try {
    ...
    for (let i = 0; i < pdfFileRecords.length; i++) {
      ...
      const internalName = fileRecord.file_subcategory || fileRecord.file_category || fileRecord.file_abbreviation || '对比页'
    ...
    if (comparisonItems.length > 0) {
      ...
      console.log(`✅ 总共添加了 ${comparisonItems.length} 个对比项到 FormData (file 为逗号拼接 URL)`)
    } else {
    ...
    if (!response) {
      throw new Error(`请求失败：所有URL都返回404或失败。最后尝试的URL: ${lastUrl}，状态码: ${lastStatusCode || 'N/A'}，错误: ${lastError?.message || '未知错误'}`)
    }
    ...
    if (isZip) {
      ...
      try {
        ...
        // 将常见的简单文本视为成功
        if (responseText === '1' || responseText.toLowerCase() === 'true' || responseText.toLowerCase() === 'ok' || responseText.toLowerCase() === 'success') {
          ...
          <el-col :span="6">
            <el-form-item>
              <el-checkbox-group v-model="caseReviewType" disabled>
                <el-checkbox label="pre" v-if="caseInfo.reviewType === 'pre'">预审案件</el-checkbox>
                <el-checkbox label="priority" v-if="caseInfo.priorityExamination === 1">优先审查</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </el-col>
          <el-col :span="6">
    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="请求书" name="request">
        ...
        <el-row :gutter="20" style="margin: 10px 0 20px;">
          <el-col :span="6">
            <el-form-item label="递交类型:">
              <el-select v-model="formData.submissionType" placeholder="请选择递交类型" style="width: 100%;">
                <el-option label="无效宣告请求书" value="无效宣告请求书"></el-option>
                <el-option label="无效宣告请求书（补充）" value="无效宣告请求书（补充）"></el-option>
                <el-option label="无效宣告请求书（修改）" value="无效宣告请求书（修改）"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-table :data="reasonTableData" style="width: 100%">
            ...
            <el-table-column prop="reason" label="无效宣告请求理由" min-width="200"></el-table-column>
            <el-table-column prop="scope" label="无效宣告请求范围" min-width="200"></el-table-column>
            <el-table-column prop="evidence" label="依据的证据" min-width="200"></el-table-column>
            <el-table-column label="操作" width="200">
              <template #default="scope">
                <el-button link type="danger" size="small" @click="deleteReason(parseInt(scope.row.sequence))">删除</el-button>
          ...
          <div style="margin-top: 20px;">
            <h4 style="margin: 0 0 10px 0; font-size: 14px; font-weight: 600; color: #333;">意见书正文</h4>
        ...
        <el-card style="margin-bottom: 10px;">
          <template #header>
            <div style="display: flex; align-items: center; gap: 20px;">
              <span>附件类型</span>
              <el-select v-model="formData.attachmentType" placeholder="请选择" style="width: 200px;">
          ...
          <el-table :data="fileTableData" style="width: 100%">
            ...
            <el-table-column prop="file_abbreviation" label="文件简称" width="300"></el-table-column>
            ...
            <el-table-column label="操作" width="120">
              <template #default="scope">
                <el-button link type="danger" size="small" @click="deleteFile(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="证件类型:">
                  <el-input :value="(requesterInfo as any)?.certificate_type || ''" placeholder="证件类型" readonly></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="国籍:">
                  <el-input :value="(requesterInfo as any)?.country || ''" placeholder="国籍" readonly></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="居住地:">
                  <el-input :value="(requesterInfo as any)?.residence || ''" placeholder="居住地" readonly></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="省:">
                  <el-input :value="(requesterInfo as any)?.province || ''" placeholder="省份" readonly></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="市:">
                  <el-input :value="(requesterInfo as any)?.city || ''" placeholder="城市" readonly></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="街道地址:">
                  <el-input :value="(requesterInfo as any)?.street_address || ''" placeholder="街道地址" readonly></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="证件号:">
                  <el-input :value="(requesterInfo as any)?.certificate_number || ''" placeholder="证件号" readonly></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="邮编:">
                  <el-input :value="(requesterInfo as any)?.postal_code || ''" placeholder="邮编" readonly></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-form :model="formData" label-width="120px">
            <el-form-item label="专利权人:">
              <el-input v-model="patentHolder" type="textarea" :rows="3" placeholder="专利权人信息"></el-input>
            </el-form-item>
          </el-form>
        </el-card>
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-form :model="formData" label-width="120px" class="compact-form">
            ...
            <el-row :gutter="15">
              <el-col :span="6">
                <el-form-item label="省:">
                  <el-input :value="(powerOfAttorneyInfo as any)?.province || ''" placeholder="省份" readonly></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item label="市:">
                  <el-input :value="(powerOfAttorneyInfo as any)?.city || ''" placeholder="城市" readonly></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item label="邮编:">
            ...
            <el-row :gutter="15">
              <el-col :span="8">
                <el-form-item label="委托日期:">
                  <el-date-picker v-model="commissionDate" type="date" placeholder="yyyy/mm/日" style="width: 100%;"></el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
        ...
        <el-card style="margin-bottom: 10px;">
          ...
          <el-table :data="powerOfAttorneyTableData" style="width: 100%; margin-bottom: 15px;">
            ...
            <el-table-column label="操作" width="120">
              <template #default="scope">
                <el-button link type="danger" size="small" @click="deletePowerOfAttorney(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-tab-pane label="待转档文件" name="pending">
        ...
        <div style="margin-bottom: 20px;">
          <el-button type="success" @click="onStartXmlConversion" style="margin-left: 10px;">启动转档XML</el-button>
        </div>
        <el-table :data="pendingFileTableData" style="width: 100%">
          <el-table-column prop="sequence" label="序号" width="80"></el-table-column>
          <el-table-column prop="original_filename" label="文件" min-width="200"></el-table-column>
          <el-table-column prop="file_subcategory" label="文件小类" width="120"></el-table-column>
          <el-table-column prop="file_description" label="文件名称" width="150"></el-table-column>
          <el-table-column prop="file_abbreviation" label="文件简称" width="120"></el-table-column>
      ...
      <el-tab-pane label="已转档文件" name="processed">
        ...
        <el-table :data="processedFileTableData" style="width: 100%">
          <el-table-column prop="sequence" label="序号" width="80"></el-table-column>
          <el-table-column prop="original_filename" label="文件" min-width="150"></el-table-column>
          <el-table-column prop="file_subcategory" label="文件小类" width="120"></el-table-column>
          <el-table-column prop="file_description" label="文件名称" width="120"></el-table-column>
          <el-table-column prop="file_abbreviation" label="文件简称" width="100"></el-table-column>
          <el-table-column prop="upload_person" label="上传人员" width="100"></el-table-column>
          ...
          <el-table-column label="操作" width="180">
            <template #default="scope">
              ...
              <el-button  v-if="!(String(scope.row.original_filename || '').toLowerCase().endsWith('.zip'))" type="success" size="small" @click="viewProcessedFile(scope.row)" plain>查看</el-button>
            </template>
          </el-table-column>
        </el-table>
