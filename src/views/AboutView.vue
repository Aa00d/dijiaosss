...
<div class="btn-group">
  ...
  <!-- 隐藏文件输入 -->
  <input ref="newApplicationFileInput" type="file" accept=".docx" @change="onNewApplicationFileChange" style="display:none" />
  <input ref="imagesInputRef" type="file" accept="image/*" multiple @change="onImagesUploadChange" style="display:none" />
</div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="6">
        <el-form-item label="申请类型:">
          ...
          >
            <el-option v-for="option in selectOptions.businessTypes" :key="option.value" :value="option.value" :label="option.label">
            </el-option>
          </el-select>
        </el-form-item>
      </el-col>
      ...
      <el-col :span="6">
        <el-form-item label="业务人员:">
          ...
          >
            <el-option v-for="p in selectOptions.personnel" :key="p.value" :label="p.label" :value="p.value" />
          </el-select>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="20">
      ...
      <el-col :span="6">
        <el-form-item label="代理机构:">
          ...
          >
            <el-option v-for="org in selectOptions.organizations" :key="org.value" :label="org.label" :value="org.value" />
          </el-select>
        </el-form-item>
      </el-col>
       <el-col :span="6">
    ...
    <el-row :gutter="20">
      <el-col :span="6">
        <el-form-item label="创新指数:">
          ...
          >
            <el-option v-for="opt in selectOptions.innovationLevels" :key="opt.value" :label="opt.label" :value="opt.value" />
          </el-select>
        </el-form-item>
      </el-col>
      <el-col :span="6">
        <el-form-item label="交底完善度:">
          ...
          >
            <el-option v-for="opt in selectOptions.disclosureLevels" :key="opt.value" :label="opt.label" :value="opt.value" />
          </el-select>
        </el-form-item>
      </el-col>
       <el-col :span="6">
...
<div class="section">
  ...
  <div class="tab-content" id="request-content">
        ...
        <el-col :span="6">
          <el-form-item >
            <el-select v-model="requestContent.fileType" placeholder="请选择">
              <el-option v-for="option in selectOptions.fileTypes" :key="option.value" :value="option.value" :label="option.label">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
    ...
    <el-form :model="requestContent" label-width="auto">
      <el-form-item>
        ...
        <el-input v-model="requestContent.abstractInsert" placeholder="指定说明书附图中的图" style="width: 200px; margin-right: 10px;"></el-input>
        <span>为摘要附图</span>
      </el-form-item>
    </el-form>
    ...
    <el-form :model="requestContent" label-width="auto">
      <el-form-item>
        <el-radio-group v-model="declarationState.application">
          <el-radio :label="true">声明本申请人对同样的发明创造在申请实用新型专利的同日申请了发明专利。</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item>
        <el-radio-group v-model="declarationState.abandon">
          <el-radio :label="true">申请人声明,放弃专利法实施细则第五十一条 规定的主动修改的权利</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item>
    ...
    <!-- 发明人信息表格 -->
    <el-table :data="requestContent.inventors || []" v-if="(requestContent.inventors || []).length > 0" style="width: 100%">
    ...
    <el-form :model="requestContent" label-width="auto">
      <el-form-item label="申请人：">
        <el-radio-group v-model="proposerSelection">
          <el-radio label="all-with-id">全体申请人请求费用减缴且已完成费用减缴资格备案</el-radio>
        </el-radio-group>
      </el-form-item>
    </el-form>

    <!-- 申请人信息表格 -->
    <el-table :data="requestContent.proposers || []" v-if="(requestContent.proposers || []).length > 0" style="width: 100%">
      <el-table-column prop="sequence" label="序号" width="80"></el-table-column>
      <el-table-column prop="name" label="名称" width="120"></el-table-column>
      <el-table-column prop="applicant_type" label="申请人类型" width="120"></el-table-column>
      <el-table-column prop="nationality" label="国籍" width="100"></el-table-column>
      ...
      <el-table-column prop="frequently_residing_address" label="地址" min-width="200"></el-table-column>
    ...
    <el-form label-width="auto">
      <el-form-item label="文件类型:">
        <el-select v-model="selectedFileType" placeholder="请选择文件类型" style="width: 250px;">
          <el-option
      ...
      <el-form-item>
        ...
        <input
          ...
          style="display: none"
        />
        <el-button type="primary" :disabled="!selectedFileType" @click="additionalFileInput?.click()">
          上传文件
        </el-button>
        <span v-if="!selectedFileType" style="margin-left: 10px; color: #909399; font-size: 12px;">
          请先选择文件类型
        </span>
      </el-form-item>
      <!-- 文件列表表格（显示真实数据库中的文件） -->
      <el-form-item v-if="uploadedAdditionalFiles.length > 0">
        <el-table :data="uploadedAdditionalFiles" style="width: 100%; margin-top: 16px;" border stripe>
          <el-table-column prop="fileName" label="文件名称" min-width="250" show-overflow-tooltip>
            <template #default="{ row }">
              <div style="display: flex; align-items: center; gap: 8px;">
                <el-icon v-if="isImageFile(row.fileType, row.fileName)" style="color: #409eff;"><Picture /></el-icon>
                <el-icon v-else style="color: #909399;"><Document /></el-icon>
                <span style="cursor: pointer; color: #409eff;" @click="openFileUrl(row.url)">{{ row.fileName }}</span>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="100" align="center">
            <template #default="{ $index }">
              <el-button type="danger" link size="small" @click="removeAdditionalFile($index)">
      ...
      <el-form-item v-if="caseInfo.powerAttorneyType == 2" label="总委托书编号:">
        <el-input v-model="caseInfo.totalPowerAttorneyNumber" style="width: 200px;"></el-input>
      </el-form-item>
      <el-form-item>
        <el-checkbox v-model="caseInfo.powerAttorneyDeclaration">声明已经与申请人签订了专利代理委托书且本表中的信息与委托书中相应信息一致</el-checkbox>
      </el-form-item>
      <el-form-item>
        <el-checkbox v-model="caseInfo.detailedApplicationHandling">代为办理该项申请</el-checkbox>
      </el-form-item>
      <el-form-item label="委托日期:">
        <el-date-picker v-model="caseInfo.powerAttorneyDate" type="date" placeholder="选择日期" style="width: 200px; margin-right: 20px;"></el-date-picker>
        <!-- 隐藏文件输入，用于委托书上传 -->
        <!-- 限制只能上传 .jpg 文件 -->
        <input
          ...
          style="display: none"
        />
        <el-button type="primary" @click="powerAttorneyInput?.click()">上传委托书</el-button>
      </el-form-item>
      <!-- 委托书文件列表表格（显示真实数据库中的委托书文件） -->
      <el-form-item v-if="uploadedPowerAttorneyFiles.length > 0">
        <el-table :data="uploadedPowerAttorneyFiles" style="width: 100%; margin-top: 16px;" border stripe>
          <el-table-column prop="fileName" label="文件名称" min-width="250" show-overflow-tooltip>
            <template #default="{ row }">
              <div style="display: flex; align-items: center; gap: 8px;">
                <el-icon v-if="isImageFile(row.fileType, row.fileName)" style="color: #409eff;"><Picture /></el-icon>
          ...
          <el-table-column label="操作" width="100" align="center">
            <template #default="{ $index }">
              <el-button type="danger" link size="small" @click="removePowerAttorneyFileByIndex($index)">
                <el-icon><Delete /></el-icon>
                删除
              </el-button>
            </template>
    ...
    <h3>代理人</h3>
    <el-table :data="requestContent.agents || []" v-if="(requestContent.agents || []).length > 0" style="width: 100%">
      <el-table-column label="序号" width="80">
    ...
    <el-input v-model="caseInfo.filedProofDocumentNumber" placeholder="请输入已备案的证明文件备案编号" style="width: 200px;"></el-input>
    ...
    <el-tab-pane label="待转档文件" name="pending-content">
      <div class="tab-content">
        <div class="btn-group">
          ...
          <el-button type="primary" :disabled="pendingFiles.length > 0" @click="triggerFileUpload">
            上传文件
            <span v-if="pendingFiles.length > 0" style="margin-left: 5px; font-size: 12px;">(已有文件，请先删除)</span>
          </el-button>
          <!-- 限制只能上传 .docx 文件，且只能上传一个文件 -->
          <input ref="fileInput" type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" style="display:none" @change="handleFileUpload" />
        </div>
        ...
        <el-table :data="pendingFiles" style="width: 100%; margin-top: 10px;">
          ...
          <el-table-column label="操作" width="160">
            <template #default="{ row, $index }">
              ...
              <el-button link type="danger" size="small" @click="removePendingFile($index)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  <!-- 加载状态 -->
  <div v-if="loading" style="height: 200px; display: flex; align-items: center; justify-content: center; background: #f5f5f5; border-radius: 4px; margin: 20px 0;">
    <div style="text-align: center;">
      <div style="width: 40px; height: 40px; border: 4px solid #f3f3f3; border-top: 4px solid #409eff; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 10px;"></div>
      <div>正在加载数据...</div>
    </div>
  </div>
...
import { uploadFileWithInternalCode, getInternalCodeByFileType, getFileTypeOptions } from '../js/InternalCode.js'
...
const loadPendingFiles = () => loadData(() => getPendingFiles(caseId.value), pendingFiles, '加载待转档文件失败')
const loadProcessFlow = () => loadData(() => getProcessFlow(caseId.value), processFlow, '加载流程过程失败')
...
const refreshProcessedFiles = async () => {
  try {
    ...
    const rawList = Array.isArray(res?.data)
      ? res.data
      : (Array.isArray(res) ? res : (res?.files || res?.fileList || []))
    ...
    for (const it of rawList) {
      ...
      // 获取文件名（支持多种可能的字段名）
      const name = String(it?.fileName || it?.realName || it?.filename || it?.originalName || it?.name || it?.real_name || '未命名')
      ...
      const fileItem: ProcessedFileItem = {
        ...
        uploadTime: String(it?.createTime || it?.uploadTime || it?.createdAt || it?.upload_time || it?.created_at || '')
      }
      ...
      ;(fileItem as any).fileBaseUrl = it?.fileBaseUrl || it?.base_url || it?.baseUrl || fileUrl || ''
...
const updateFieldDisabled = () => {
  ...
  const fields = [
    'inventionName', 'applicationNumber', 'originalApplicationDate',
    'internalNumber', 'applicantName', 'institutionCode',
    'organizationName', 'signatureDate', 'submissionType', 'abstractInsert',
    'applicationType', 'divisionApplicationNumber', 'businessPerson',
    'deferredExamination', 'innovationIndex', 'disclosureCompleteness',
    'createTime', 'updateTime', 'caseId'
  ]
    ...
    results.forEach((result: any, index: number) => {
      if (result?.success && result?.data) {
        ...
        const fileInfo: FileInfo = {
          ...
          uploadTime: data.createTime || data.upload_time || new Date().toLocaleString('zh-CN', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
        ...
        // 检查是否已存在（避免重复添加）
        const exists = pendingFiles.value.some(f => f.id === fileInfo.id || f.fileName === fileInfo.fileName)
        if (!exists) {
...
const handleAdditionalFileUpload = async (event: Event) => {
  ...
  try {
    ...
    for (let index = 0; index < results.length; index++) {
      ...
      if (result?.success && result?.data) {
        ...
        const uploadedFile: UploadedFile = {
          ...
          uploadTime: data.createTime || new Date().toLocaleString('zh-CN', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
        ...
        if (!exists) {
          ...
          console.log('✅ 文件已添加到列表（数据库ID:', uploadedFile.id, '）:', uploadedFile.fileName)
    ...
    // 所有文件处理完成后，强制触发DOM更新
    await nextTick()
    console.log('✅ DOM已更新，表格应该显示，uploadedAdditionalFiles.length =', uploadedAdditionalFiles.value.length)
    ...
    if (successCount === fileArray.length) {
      ...
      const failedErrors = results
        .map((r: any) => !r.success ? r.error : null)
        .filter(Boolean)
    ...
    console.log('✅ 当前 uploadedAdditionalFiles 数量:', uploadedAdditionalFiles.value.length)
    console.log('✅ 当前 uploadedAdditionalFiles 列表:', uploadedAdditionalFiles.value.map(f => ({ id: f.id, fileName: f.fileName })))
    ...
    console.log('✅ 表格应该显示，因为 uploadedAdditionalFiles.length =', uploadedAdditionalFiles.value.length)
  } catch (err) {
...
const handlePowerAttorneyUpload = async (event: Event) => {
  ...
  try {
    ...
    if ((result as any)?.success && (result as any)?.data) {
      ...
      // 检查是否已存在（避免重复添加）
      const existsInAdditional = uploadedAdditionalFiles.value.some(f => f.id === uploadedFile.id)
      const existsInPowerAttorney = uploadedPowerAttorneyFiles.value.some(f => f.id === uploadedFile.id)
      ...
      if (!existsInAdditional && !existsInPowerAttorney) {
        ...
        console.log(' 委托书已添加到列表（数据库ID:', uploadedFile.id, '）:', uploadedFile.fileName)
    ...
    if (file.id) {
      ...
      if (file.url) {
        ...
        if (urlIndex !== -1) {
          ...
          const fullUrlIndex = pendingFileUrls.value.findIndex(u => u.includes(url) || url.includes(u))
          if (fullUrlIndex !== -1) {
...
const removePowerAttorneyFileByIndex = async (index: number) => {
  try {
    ...
    // 确认删除
    await ElMessageBox.confirm(
      `确定要删除委托书"${file.fileName}"吗？删除后无法恢复。`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
const newApplicationFileName = computed(() => newApplicationFile.value ? newApplicationFile.value.name : '')
    ...
    if (result.success && result.files && Array.isArray(result.files)) {
      ...
      result.files.forEach((file: any) => {
        const uploadedFile: UploadedFile = {
          ...
          fileType: '', // 从文件名推断类型
          uploadTime: file.createTime || new Date().toLocaleString('zh-CN', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
        ...
        // special === '2' 或 2 → 委托书
        // special === '3' 或 3 → 待转档文件
        const fileSpecial = file.special === '1' || file.special === 1 ? '1' :
                           file.special === '2' || file.special === 2 ? '2' :
                           file.special === '3' || file.special === 3 ? '3' : null

        // 判断是否为委托书（优先根据special字段，其次根据文件名或fileCategoryMinor）
        const isPowerAttorney = fileSpecial === '2' ||
        ...
        const isPendingFile = fileSpecial === '3' ||
        ...
        if (isPowerAttorney) {
          ...
          if (url) {
            ...
            if (url && !pendingFileUrls.value.includes(url)) {
              pendingFileUrls.value.push(url)
              console.log('✅ 从数据库加载时，待转档文件URL已添加到 pendingFileUrls (完整URL格式):', url)
            }
          }
          ...
          if (!existingAdditionalFileIds.has(uploadedFile.id)) {
            ...
            console.log('✅ 从数据库加载附加文件 (special=1):', uploadedFile.fileName, 'ID:', uploadedFile.id)
          }
          ...
          if (url) {
            ...
            if (url && !attachedFileUrls.value.includes(url)) {
              attachedFileUrls.value.push(url)
              console.log('✅ 从数据库加载时，附加文件URL已添加到 attachedFileUrls (完整URL格式):', url)
            }
          }
        } else {
...
function buildFunctionalPayload() {
  ...
  const rawDate = ((requestContent as any).date || caseInfo.signatureDate || formFunctional.petition.date || '').trim()
  ...
  return {
    ...
    applicationFiledCase: (requestContent as any).applicationFiledCase || caseInfo.internalNumber || '',
    dateApplication: (() => {
      // 获取原始值
      const rawValue = (requestContent as any).dateApplication || caseInfo.originalApplicationDate || ''
...
function buildPetitionSqlPayload() {
  ...
  if (formFunctional.petition.signature && Array.isArray(formFunctional.petition.signature)) {
    signatureValue = formFunctional.petition.signature.filter((s: any) => s && String(s).trim() !== '')
  }
  ...
  const payload: any = {
    ...
    nameOfOrganization: caseInfo.organizationName || formFunctional.petition.nameOfOrganization || '',
...
async function submitNewApplication() {
  ...
  try {
    ...
    console.log('✅ images字段已添加到FormData (URL数量:', imageUrls.value.length, '):', imageUrls.value)
    ...
    console.log('✅ fileAttached字段已添加到FormData (URL数量:', attachedFileUrls.value.length, '):', attachedFileUrls.value)
    ...
    console.log('✅ file字段已添加到FormData (URL数量:', pendingFileUrls.value.length, '):', pendingFileUrls.value)
    ...
    const curl = [
      ...
      attachedFileUrls.value.length > 0 ? `-F "fileAttached=${JSON.stringify(attachedFileUrls.value)}"` : '',
    ...
    if (contentType.includes('application/json')) {
    ...
    } else if (contentType.includes('application/zip') || contentType.includes('application/x-zip-compressed') || contentType.includes('octet-stream')) {
      // ZIP文件响应 - 直接上传到数据库
      ...
      try {
        ...
        const zipFileUrl = (uploadResult as any)?.url ||
      ...
      if (data instanceof Blob) {
        try {
          ...
          try {
            ...
            // 提取错误消息，处理嵌套的异常信息
            let extractedMessage = jsonData.message || jsonData.error || jsonData.msg || `服务器错误 (${status})`
            ...
            console.error('functionalString 预览:', JSON.stringify(functionalPayload).substring(0, 500))
            console.error('powerAttorneyString 预览:', JSON.stringify(powerAttorneyPayload).substring(0, 500))
            console.error('petitionSqlString 预览:', JSON.stringify(petitionSqlPayload).substring(0, 500))
            ...
            if (errorMessage.includes('JSON') || errorMessage.includes('格式')) {
              ...
              if (functionalPayload && powerAttorneyPayload && petitionSqlPayload) {
                ...
                console.error('powerAttorneyString:', JSON.stringify(powerAttorneyPayload, null, 2))
                ...
                console.error('fileAttached (URL数组):', JSON.stringify(attachedFileUrls.value, null, 2))
                ...
                console.error('powerAttorneyString:', JSON.stringify(errorPowerAttorneyPayload, null, 2))
                console.error('petitionSqlString:', JSON.stringify(errorPetitionSqlPayload, null, 2))
                console.error('file (URL数组):', JSON.stringify(pendingFileUrls.value, null, 2))
                console.error('fileAttached (URL数组):', JSON.stringify(attachedFileUrls.value, null, 2))
                console.error('images (URL数组):', JSON.stringify(imageUrls.value, null, 2))
              }
...
const saveForm = async () => {
  try {
    ...
    const payload = {
      ...
      personal_annual_income: 120000.50, // 默认年收入
...
function applyCaseSummaryToForm(info: any) {
  ...
  caseInfo.applicationType = info.applicationType === '0' ? 0 : (info.applicationType === '1' ? 1 : (info.applicationType === '2' ? 2 : caseInfo.applicationType))
  ...
  caseInfo.divisionApplicationNumber = info.doubleReport ?? caseInfo.divisionApplicationNumber ?? ''
  caseInfo.originalApplicationDate = info.sameDayFiling ?? caseInfo.originalApplicationDate ?? ''
  caseInfo.abstractInsert = info.disclosureCompleteness ? Number(info.disclosureCompleteness) : caseInfo.abstractInsert
  caseInfo.businessPerson = info.businessPersonName ?? caseInfo.businessPerson ?? ''
  caseInfo.innovationIndex = info.innovationIndex ? String(info.innovationIndex) : caseInfo.innovationIndex ?? ''
  caseInfo.disclosureCompleteness = info.disclosureCompleteness ?? caseInfo.disclosureCompleteness ?? ''
  ...
  requestContent.applicationType = info.applicationType === '0' ? 0 : (info.applicationType === '1' ? 1 : (info.applicationType === '2' ? 2 : requestContent.applicationType))
  ...
  requestContent.applicationNumber = info.applicationNumber ?? requestContent.applicationNumber ?? ''
  requestContent.abstractInsert = info.disclosureCompleteness ? Number(info.disclosureCompleteness) : requestContent.abstractInsert
}
...
function processApiResponse(data: any) {
  ...
  caseInfo.originalApplicationDate = data.originalApplicationDate || data.original_application_date || ''
  ...
  caseInfo.totalPowerAttorneyNumber = data.totalPowerAttorneyNumber || data.total_power_attorney_number || ''
  caseInfo.powerAttorneyDate = data.powerAttorneyDate || data.power_attorney_date || ''
  caseInfo.powerAttorneyDeclaration = data.powerAttorneyDeclaration || data.power_attorney_declaration || false
  caseInfo.detailedApplicationHandling = data.detailedApplicationHandling || data.detailed_application_handling || false
  caseInfo.powerAttorneyFilename = data.powerAttorneyFilename || data.power_attorney_filename || ''
  caseInfo.divisionApplicationNumber = data.divisionApplicationNumber || data.division_application_number || ''
  ...
  // 更新声明状态
  declarationState.deferredexamination = data.deferredExamination || data.deferred_examination || false
  ...
  try {
    ...
    if (inventorsData) {
      const inventors = typeof inventorsData === 'string' ? JSON.parse(inventorsData) : inventorsData
    ...
    if (proposersData) {
      const proposers = typeof proposersData === 'string' ? JSON.parse(proposersData) : proposersData
      if (Array.isArray(proposers)) {
        requestContent.proposers = proposers.map((prop: any, idx: number) => ({
          id: idx + 1,
          name: prop.name || prop.proposer_name || '',
          sequence: prop.order !== undefined ? prop.order : (prop.order_num !== undefined ? prop.order_num : idx + 1),
    ...
    if (priorityData) {
      const priorityInfo = typeof priorityData === 'string' ? JSON.parse(priorityData) : priorityData
