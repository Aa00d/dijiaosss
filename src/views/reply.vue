<template>
  <div class="respond-view">
      ...
      <div class="form-row">
        <el-form-item label="代理机构:">
          <el-select v-model="caseInfo.agencyName" disabled>
            <el-option label="成都睿道智诚专利代理有限公司" value="成都睿道智诚专利代理有限公司"></el-option>
          </el-select>
        </el-form-item>
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="请求书" name="request-content">
          ...
          <div class="basic-info-container">
            <div class="basic-info-options">
              <el-radio-group v-model="notificationType">
                <el-radio label="0">
                  针对国家知识产权局于<el-date-picker v-model="notificationDate" type="date" size="small" />
                  发出的<el-input v-model="notificationName" placeholder="第一次审查意见通知书" size="small" style="width: 200px;" />
                  （发文号<el-input v-model="notificationSerial" placeholder="CN2025-001" size="small" style="width: 150px;" />）陈述意见
                </el-radio>
                <el-radio label="1">
                  针对国家知识产权局于<el-date-picker v-model="notificationDate" type="date" size="small" />
                  发出的<el-input v-model="notificationName" placeholder="补充审查意见通知书" size="small" style="width: 200px;" />
                  （发文号<el-input v-model="notificationSerial" placeholder="CN2025-002" size="small" style="width: 150px;" />）补充陈述意见
                </el-radio>
                <el-radio label="2">
                  针对国家知识产权局于<el-date-picker v-model="notificationDate" type="date" size="small" />日
                  发出的药品专利权期限补充审查意见通知书
                  （发文号<el-input v-model="notificationSerial" placeholder="CN2025-003" size="small" style="width: 150px;" />）陈述意见
                </el-radio>
          ...
          <div class="section">
            ...
            <div class="form-row">
              <el-input v-model="statementContent" :disabled="isStatementFileMode" type="textarea" :rows="5" placeholder="请输入意见陈述内容..." style="width: 100%;"></el-input>
            </div>
            <div class="form-row">
          ...
          <div class="section">
            <div class="form-row">
              ...
              <el-form-item label="">
                <el-select v-model="uploadType" @change="handleUploadTypeChange" :options="getAvailableUploadTypes()">
                  <!-- 动态选项将通过计算属性生成 -->
                </el-select>
              </el-form-item>
              <el-form-item label="">
                <el-select v-model="proofDocType" placeholder="其他证明文件-扫描件（普通）A100108">
                  <el-option label="其他证明文件-扫描件（普通）A100108" value="其他证明文件-扫描件（普通）A100108"></el-option>
                </el-select>
              </el-form-item>
              <!-- 新增：图片中的下拉框数据 -->
              <el-form-item label="">
                <el-select v-model="otherProofSelect" placeholder="后台配置选择1" @change="handleProofSelectChange">
                  <el-option label="在先申请文件副本中文题录" value="在先申请文件副本中文题录"></el-option>
                  <el-option label="遗传资源" value="遗传资源"></el-option>
                  <el-option label="生物材料样品保藏及存活证明中文题录" value="生物材料样品保藏及存活证明中文题录"></el-option>
                  <el-option label="优先权转让证明中文题录" value="优先权转让证明中文题录"></el-option>
                </el-select>
              </el-form-item>
            </div>
            <div class="form-row">
    ...
    <!-- 上传文件弹窗 -->
    <el-dialog :title="uploadModalTitle" v-model="uploadModalVisible" width="400px" @closed="resetUploadModal">
      <div style="display: flex; justify-content: center; align-items: center;">
        <div class="upload-area" @click="triggerFileInput">
          <input type="file" ref="fileInputRef" style="display: none;" :accept="uploadAccept" @change="handleFileSelect">
...
const combinedUploadName = computed(() => `${(uploadMode.value || '').trim()}${(uploadType.value || '').trim()}`)
...
const uploadZipBytes = async (arrayBuffer) => {
  try {
    ...
    if (!processesId || !caseId) {
      const errorMessage = 'URL中缺少必需的查询参数 case_processes_id 或 case_id，无法上传ZIP二进制流'
      ...
      if (result.files.length > 0) {
        ...
        statementFiles.value = otherFiles
          ...
          .filter(file => {
            ...
            return ['权利要求书', '说明书', '说明书附图', '修改对照页', '修改对照','其他证明文件-扫描件（普通）'].includes(minorCategory);
          })
          .map((file, index) => ({
            id: file.id || index + 1,
            fileName: file.fileName || '',
...
const fetchReviewResponseById = async () => {
  ...
  const { processesId, caseId } = getParamsFromUrl();
  const response = await fetch(`http://106.52.57.109:8082/api/review-response/by-case?case_processes_id=${encodeURIComponent(processesId || case_processes_id.value)}&case_id=${encodeURIComponent(caseId || case_id.value)}`, { method: 'GET' })
...
const applyUserDataToPage = (userData) => {
  try {
    ...
    const data = Array.isArray(userData.data) && userData.data.length > 0 ? userData.data[0] : userData.data || {};
    ...
    caseInfo.applicationNumber = data.filedProofNumber || data.applicationNumber || caseInfo.applicationNumber;
    ...
    caseInfo.processItem = data.multipleApplicationNumbers || data.processItem || caseInfo.processItem;
    ...
    if (!caseInfo.internalDeadline) {
      const officialDate = caseInfo.officialDeadline ? new Date(caseInfo.officialDeadline) : new Date();
...
async function applyCaseSummaryFromApi(id = 1001) {
  try {
    ...
    if (caseInfo && typeof caseInfo === 'object') {
      ...
      caseInfo.proposalName = pickNonEmpty(info?.proposalName, info?.case_name) ?? caseInfo.proposalName
      caseInfo.caseNumber = pickNonEmpty(info?.projectNo, info?.project_no) ?? caseInfo.caseNumber
      caseInfo.applicationNumber = pickNonEmpty(info?.applicationNumber, info?.application_number) ?? caseInfo.applicationNumber
      caseInfo.caseStatus = pickNonEmpty(info?.caseStatus, info?.case_status) ?? caseInfo.caseStatus
      ...
      caseInfo.processItem = pickNonEmpty(info?.processItem, info?.process_name) ?? caseInfo.processItem
      ...
      caseInfo.officialDeadline = pickNonEmpty(info?.officialDeadline, info?.official_deadline) ?? caseInfo.officialDeadline
      caseInfo.internalDeadline = pickNonEmpty(info?.internalDeadline, info?.internal_deadline) ?? caseInfo.internalDeadline
      caseInfo.customerName = pickNonEmpty(info?.customerName, info?.customer_name) ?? caseInfo.customerName
      caseInfo.techLeader = pickNonEmpty(info?.techLead, info?.tech_leader_name) ?? caseInfo.techLeader
...
const uploadAccept = computed(() => currentUploadTarget.value === 'comparison' ? '.docx' : '.doc,.docx')
  ...
  if (selectedFile.value) {
    ...
    try {
      ...
      if (currentUploadTarget.value === 'comparison') {
        ...
        try {
          ...
          if (filesResult.data && filesResult.data.files && Array.isArray(filesResult.data.files)) {
            filesResult.data.files.forEach(fileInfo => {
              ...
              // 只包含符合要求的文件小类
              if (['权利要求书', '说明书', '说明书附图', '修改对照页', '修改对照', '其他证明文件-扫描件（普通）'].includes(minorCategory)) {
                fileNameToUrl[fileInfo.fileName] = fileInfo.url || '';
                console.log(`🔍 已映射文件: ${fileInfo.fileName} -> ${fileInfo.url}, 小类: ${minorCategory}`);
              }
            });
          }
        ...
        console.log(`🔍 陈述文件上传成功，将作为Statement[0].file提交，name值将从uploadType下拉框获取: ${fileTypeDescription}`)
      }
...
// 监听上传模式变化，重置上传类型
watch(() => uploadMode.value, (newMode) => {
  if (newMode === '修订模式') {
...
async function onSave() {
  try {
    ...
    // 发送保存请求，使用URL查询参数
    const response = await fetch(`http://106.52.57.109:8082/api/review-response/save?${queryParams}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    try {
      ...
      if (type === 'comparison') {
        ...
        if (uploadMode.value === '修订模式') {
          ...
          console.log('🔍 修订模式 - 修改对照页上传成功:', file.name, 'URL:', fileNameToUrl[file.name], '将作为comparisonPage字段提交');
        } else if (uploadMode.value === '替换页模式') {
          // 替换模式：允许上传多个文件
          ...
          if (fileName.includes('权利要求') || fileName.includes('claim')) {
            ...
            fileType = '说明书';
            fileShortName = '说明书';
          } else if (fileName.includes('附图') || fileName.includes('figure') || fileName.includes('drawing')) {
            fileType = '说明书附图';
            fileShortName = '附图';
          ...
          console.log('🔍 替换页模式 - 文件上传成功:', file.name, '文件类型:', fileType, 'URL:', fileNameToUrl[file.name]);
...
async function submitReviewResponse(actionType = 'save') {
  ...
  try {
    ...
    } else if (filesResult.success && filesResult.data && filesResult.data.files && Array.isArray(filesResult.data.files)) {
  ...
  const comparisonPageFiles = allFiles.filter(f => {
    ...
    // 查找文件小类为"意见陈述书正文"的文件
    return minorCategory === '意见陈述书正文' ||
           fileName.toLowerCase().match(/\.(docx|doc|pdf)$/) &&
           (fileName.includes('意见陈述') || fileName.includes('comparison') || fileName.includes('对比'));
  });
  ...
  comparisonPageFiles.forEach(file => {
    if (file && file.url) {
      ...
      console.log(`🔍 comparisonPage文件信息: fileName=${file.fileName}, fileCategoryMinor=${file.fileCategoryMinor || file.file_category}`);
    }
  });
  ...
  if (!isRegulationAmendment) {
    ...
    if (validType === 0 || validType === 1) {
      notification.name = notificationName.value || (validType === 0 ? '第一次审查意见通知书' : '补充审查意见通知书');
    } else if (validType === 2) {
  ...
  const statementFilesList = allFiles.filter(f => {
    ...
    return fileName.toLowerCase().match(/\.(doc|docx|pdf)$/) ||
           ['权利要求书', '说明书', '说明书附图', '修改对照页', '修改对照', '其他证明文件-扫描件（普通）'].includes(minorCategory);
  });

  console.log(`🔍 找到${statementFilesList.length}个符合Statement要求的文件（排除"意见陈述书正文"）`);
  ...
  if (fileUrls.length > 0) {
    ...
    console.log(`🔍 所有文件URL已合并到Statement[0].fileUrl中，共${fileUrls.length}个文件，格式为JSON数组字符串`);
  } else {
    ...
    if (statementFile.value) {
      ...
      console.log(`🔍 添加备选Statement参数: name=${statementName}, file=${statementFile.value.name}`);
      statementIndex++;
      hasStatement = true;
    }

    // 根据上传模式添加其他文件（排除文件小类为"意见陈述书正文"的文件）
    const validFiles = additionalFiles.value.filter(f =>
      f.rawFile && (f.fileCategory !== '意见陈述书正文')
    );
    ...
    if (!hasStatement) {
      ...
      formData.append('Statement[0].name', '修改对照');
      console.log(`🔍 添加空的Statement参数以避免后端空指针异常: fileUrl=${emptyFileUrls}，name="修改对照"`);
    }
  }
  ...
  try {
    ...
    const imageFiles = allFiles.filter(f => {
      ...
      console.log(`🔍 检查文件: fileName=${fileName}, minorCategory=${minorCategory}, hasUrl=${hasUrl}`);
    ...
    imageFiles.forEach((file, index) => {
      if (file.url && typeof file.url === 'string') {
        ...
        if (url && url.length > 0) {
          imageUrls.push(url);
          console.log(`✅ reply.vue成功添加匹配文件${index + 1}: fileName=${file.fileName || '未知'}, URL=${url}`);
        } else {
          console.warn(`⚠️ URL处理后为空: fileName=${file.fileName || '未知'}, 原始URL=${file.url}`);
        }
      } else {
  ...
  try {
    ...
    console.log('准备发送请求 -> actionType:', actionType, 'url:', 'http://47.108.144.113:9111/api/word/statement-opinion/xml')
    console.log('FormData参数摘要:')
    console.log('- comparisonPage:', comparisonFile.value ? comparisonFile.value.name : '未提供（JSON数组字符串形式）')
    console.log('- Statement[].fileUrl[]:', statementFile.value ? `${statementFile.value.name} (数组形式)` : '未提供');
  console.log('- Statement列表长度:', Math.max(1, additionalFiles.value.length));
    console.log('- docx:', comparisonFile.value ? '未提供（comparisonPage已存在）' : (statementContent.value.trim() ? `已提供 - 内容: ${statementContent.value.trim()}` : '未提供'))
    ...
    try {
      ...
      if (contentType.includes('application/json') || contentType.includes('text/json') || contentType.includes('application/problem+json')) {
        ...
        if (text) {
          try {
            ...
            else if (parsed.data && parsed.data.response2 !== undefined) console.log('response2 (in data):', parsed.data.response2)
          } catch {
            // 非 JSON 文本或二进制内容
            console.log('响应为非 JSON 文本/二进制，response 状态码:', response.status, 'content-type:', contentType)
          }
        } else {
          console.log('响应可能为二进制（blob），status:', response.status, 'content-type:', contentType)
        }
      }
    } catch (e) {
      ...
      try {
        ...
        if (uploadResult.success) {
          ...
          if (skippedCount > 0) {
            ...
            // 检查是否有"未解析到内部代码"的文件
            const unparsedFiles = uploadResult.skipped.filter(item => item.reason === '未解析到内部代码')
          ...
          // 如果有跳过的文件，添加到错误消息中
          if (uploadResult.skipped && Array.isArray(uploadResult.skipped) && uploadResult.skipped.length > 0) {
            const skippedFiles = uploadResult.skipped.map(item => item.file_name).join('、')
            const skippedReasons = uploadResult.skipped.map(item => `${item.file_name}(${item.reason})`).join('、')
...
const deleteFile = async (id) => {
  try {
    ...
    // 删除成功后更新本地文件列表
    // 更新additionalFiles列表
    const additionalIndex = additionalFiles.value.findIndex(file => file.id === id || file.file_id === id)
