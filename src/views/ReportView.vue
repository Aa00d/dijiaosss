        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item label="处理事项ID:">
              <el-input v-model="caseInfo.case_processes_id" placeholder="处理事项ID" readonly></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <div class="tab-content">
              ...
              <el-row :gutter="20">
                <el-col :span="6">
                  <el-form-item label="递交类型:">
                    <el-select v-model="evaluationReportData.submissionType" placeholder="后台配置选择">
                      <el-option label="后台配置选择" value=""></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
            ...
            <div class="section">
              ...
              <div class="form-row">
               ...
               <!-- 利害关系人专用字段：许可方和被许可方 -->
               <div v-if="evaluationReportData.requesterType === 'interestedParty'" class="section" style="margin-bottom: 20px;">
                 <div class="form-row">
                 ...
                 <!-- 多选框选项 -->
                 <div class="form-row" style="margin-top: 15px;">
                   <el-checkbox v-model="evaluationReportData.grantLicenseeSueRight">授予被许可人起诉权</el-checkbox>
                 </div>
                 <div class="form-row">
                   <el-checkbox v-model="evaluationReportData.allPartiesSigned">声明上述当事人已全部签字或盖章</el-checkbox>
                 </div>
                 <div class="form-row">
                   <el-checkbox v-model="evaluationReportData.abstractConsistent">请求人提供的中文题录与专利权评价报告证明文件中的信息是一致的</el-checkbox>
                 </div>
                   ...
                   <el-table :data="attachmentFiles" border style="width: 100%;">
                     ...
                     <el-table-column prop="fileCategory" label="文件小类">
                       <template #default="scope">
                         <el-input v-model="scope.row.fileCategory" placeholder="请输入文件小类" size="small" />
                       </template>
                     </el-table-column>
                     <el-table-column prop="fileTitle" label="文件名称">
                       <template #default="scope">
                         <el-input v-model="scope.row.fileTitle" placeholder="请输入文件名称" size="small" />
                       </template>
                     </el-table-column>
                     <el-table-column prop="fileShortName" label="文件简称">
                       <template #default="scope">
                         <el-input v-model="scope.row.fileShortName" placeholder="请输入文件简称" size="small" />
                       </template>
                     </el-table-column>
                     <el-table-column prop="uploader" label="上传人员">
                       <template #default="scope">
                         <el-input v-model="scope.row.uploader" placeholder="请输入上传人员" size="small" />
                       </template>
                     </el-table-column>
                     <el-table-column prop="uploadTime" label="上传时间">
                       <template #default="scope">
                         <el-input v-model="scope.row.uploadTime" placeholder="请输入上传时间" size="small" />
                       </template>
                     </el-table-column>
                     <el-table-column label="操作" width="100">
                       <template #default="scope">
                         <el-button type="primary" size="small" @click="uploadAttachment">上传</el-button>
                       </template>
                     </el-table-column>
                   </el-table>
                 </div>
               ...
               <!-- 被控侵权人专用字段：原告被告 -->
               <div v-if="evaluationReportData.requesterType === 'accusedInfringer'" class="section" style="margin-bottom: 20px;">
                 <div class="form-row">
               ...
               <!-- 附件选择 -->
               <div class="form-row" style="margin-top: 20px;">
                 <el-select v-model="evaluationReportData.attachmentType" placeholder="上传附件" style="width: 400px;">
                 ...
                 <el-button type="primary" style="margin-left: 10px;" @click="uploadAttachment">上传</el-button>
            ...
            <div class="section">
              ...
              <div class="form-row" style="display: flex; align-items: center; gap: 10px; margin-top: 15px;">
                ...
                <span style="margin-left: 10px; color: #666;">{{ evaluationReportData.powerOfAttorneyFile || '张三科技公司委托书.jpg' }}</span>
              </div>
            ...
            <el-table :data="processedFiles" style="width: 100%">
              ...
              <el-table-column label="操作" width="120">
                <template #default="{ row }">
                  ...
                  <el-button link type="primary" size="small" @click="viewProcessedFile(row)">查看</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
// 评价报告相关API
const getEvaluationReport = async (id: string): Promise<any> => apiCall(`/evaluation-report/${id}`, {
  id: 0, createTime: '', updateTime: '', submissionType: '',
  patentHolderRepresentative: '', evaluationRequester: '',
  requesterType: 'patentHolder', plaintiffs: '', defendants: '', licensors: '', licensees: '',
  enablePowerOfAttorney: false, powerOfAttorneyStatement1: false, powerOfAttorneyStatement2: false,
  abstractConsistent: false, grantLicenseeSueRight: false, allPartiesSigned: false,
  entrustedParty: '', agencyCode: '', agent1Info: '{}', agent2Info: '{}',
  entrustDate: '', powerOfAttorneyFile: '', proofFileRecordNumber: '', attachmentType: ''
})
    ...
    try {
      ...
      if (uploadResult.success) {
        ...
        if (uploadResult.data) {
          let url = uploadResult.data.url || uploadResult.data.base_url || uploadResult.data.baseUrl || ''
          if (url) {
            ...
            console.log('📋 URL格式验证:', {
              original: uploadResult.data.url || uploadResult.data.base_url || uploadResult.data.baseUrl,
              processed: url,
...
const fetchFileList = async () => {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/files/list-by-submission?${queryParams.toString()}`, {
      method: 'GET',
      headers: {
    ...
    if (files.length > 0) {
      ...
      console.log('📋 原始文件列表的special值:', files.map((f: any) => ({ fileName: f.fileName, special: f.special, specialType: typeof f.special })))
      ...
      const filteredFiles = files.filter((file: any) => {
        ...
        const isSpecial666 = special === '666' ||
                            special === 666 ||
                            String(special) === '666' ||
      ...
      console.log('📊 筛选统计: 总数=', files.length, ', 排除special=666后数量=', filteredFiles.length)
...
const fetchProcessedFiles = async () => {
  ...
  try {
    ...
    if (files.length > 0) {
      console.log('📋 原始文件列表:', files)
      console.log('📋 原始文件列表的special值:', files.map((f: any) => ({ fileName: f.fileName || f.file_name, special: f.special, specialType: typeof f.special })))
      ...
      const filteredFiles = files.filter((file: any) => {
        ...
        if (!isSpecial666) {
          console.log(`⏭️ 跳过文件（special≠666）: ${file.fileName || file.file_name || '未知'}, special值: ${special}`)
        } else {
          console.log(`✅ 保留文件（special=666）: ${file.fileName || file.file_name || '未知'}, special值: ${special}`)
        }
      ...
      processedFiles.value = filteredFiles.map((file: any, index: number) => ({
        ...
        fileType: file.fileCategoryMinor || file.fileCategoryMajor || file.file_category_minor || file.file_category_major || '',
        fileTitle: file.fileTitle || file.file_title || '',
        fileShortName: file.fileShortName || file.file_short_name || '',
        uploader: file.uploader || '',
        uploadTime: file.createTime ? formatDate(file.createTime) : (file.create_time ? formatDate(file.create_time) : ''),
        url: file.url || ''
      }))
      ...
      if (filteredFiles.length === 0) {
        ...
        console.log('所有文件的 special 值:', files.map((f: any) => ({ fileName: f.fileName, special: f.special })))
      }
    } else {
      console.warn('⚠️ 响应数据格式不正确或没有文件')
      console.log('响应数据结构:', JSON.stringify(data, null, 2))
...
const deleteFile = async (fileId: number) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`删除失败: ${response.status} ${response.statusText}${errorText ? ' - ' + errorText : ''}`)
    }
...
const saveData = async () => {
  try {
    ...
    const url = `http://106.52.57.109:8082/api/evaluation-report/save?case_processes_id=${case_processes_id}&case_id=${case_id}&submission_page=评价报告`
...
const deleteData = async () => {
  try {
    ...
    if (confirmResult === 'confirm') {
      ...
      const case_id = 1001
      const url = `http://106.52.57.109:8082/api/evaluation-report/delete?case_processes_id=${case_processes_id}&case_id=${case_id}`
...
const submitData = async (): Promise<void> => {
  ...
  try {
          ...
          try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              parsed.forEach((url: string, index: number) => {
                ...
                console.log(`      ${key}[${index}]: ${url} ${hasHttps ? '✅ 包含https://' : '❌ 缺少https://'}`)
              })
            } else {
              console.log(`    ${key}: 空数组 []`)
            }
    ...
    if (blob.size === 0) {
      ...
      if (isZipFile || contentType.includes('application/zip') || contentType.includes('application/x-zip-compressed') || contentType.includes('octet-stream')) {
        // 是zip文件，处理二进制流
        ...
        try {
          ...
          if (caseProcessSubmissionId && !urlParams.case_process_submission_id) {
            ...
            console.log('✅ 已将 case_process_submission_id 保存到 urlParams:', urlParams.case_process_submission_id)
          }
          ...
          if (urlParams.case_processes_id && urlParams.case_id) {
            ...
            if ((window as any).uploadedZipFileUrl && processedFiles.value.length > 0) {
              ...
              console.log('✅ 已使用上传返回的URL更新文件列表:', (window as any).uploadedZipFileUrl)
            }
...
const executeIdQuery = async () => {
  ...
  try {
    ...
    ElMessage.success(`查询成功 - 处理事项ID: ${idQueryForm.case_processes_id}, 案件ID: ${idQueryForm.case_id}`)
    closeIdQueryModal()
  } catch (err) {
