...
// 通用API工具函数
const apiCall = async (endpoint: string, defaultData: any = {}, params?: Record<string, string>) => {
  try {
    ...
    return data.data && Array.isArray(data.data) && data.data.length > 0 ? data.data[0] : defaultData
  } catch (error) {
...
const loadPriorityData = async () => {
  try {
    console.log('开始加载优先权数据，案件ID:', priorityData.value.caseId, '处理事项ID:', priorityData.value.processItemId)
...
const loadCaseSummaryById = async (id?: string) => {
  try {
    ...
    if (summary.applicationType !== undefined) priorityData.value.applicationType = summary.applicationType || ''
    if (summary.caseName !== undefined) priorityData.value.caseName = summary.caseName || ''
    if (summary.proposalName !== undefined) priorityData.value.proposalName = summary.proposalName || priorityData.value.caseName || ''
    if (summary.projectNo !== undefined) priorityData.value.caseNumber = summary.projectNo || ''
    if (summary.processItem !== undefined) priorityData.value.processItem = summary.processItem || ''
    if (summary.doubleReport !== undefined) priorityData.value.doubleReport = summary.doubleReport || ''
    if (summary.businessPersonName !== undefined) priorityData.value.businessPerson = summary.businessPersonName || ''
    if (summary.customerName !== undefined) priorityData.value.clientName = summary.customerName || ''
    ...
    if (summary.sameDayFiling !== undefined) priorityData.value.sameDay = summary.sameDayFiling || ''
    if (summary.innovationIndex !== undefined) priorityData.value.innovationIndex = summary.innovationIndex || ''
    if (summary.disclosureCompleteness !== undefined) priorityData.value.completeness = summary.disclosureCompleteness || ''
    if (summary.agency !== undefined) priorityData.value.agency = summary.agency || ''
    if (summary.applicationNumber !== undefined) priorityData.value.applicationNumber = summary.applicationNumber || ''
    ...
    if (summary.officialDeadline !== undefined) priorityData.value.officialDeadline = summary.officialDeadline || ''
    if (summary.internalDeadline !== undefined) priorityData.value.internalDeadline = summary.internalDeadline || ''
...
const loadProcessedFiles = async () => {
  try {
    ...
    if (response && response.success && Array.isArray(response.files)) {
      ...
      console.warn('获取的文件列表数据格式不正确，期望格式: {success: true, files: [...]}, 实际格式:', Object.keys(response || {}));
...
const { caseProcessesId, caseId } = getParamsFromUrl();
    const res = await fetch(`http://106.52.57.109:8082/api/evaluation-report/save?case_processes_id=${caseProcessesId || priorityData.value.processItemId}&case_id=${caseId || priorityData.value.caseId}&submission_page=优先权`, {
      method: 'POST',
...
const onDelete = async () => {
  try {
    ...
    const apiUrl = 'http://106.52.57.109:8082/api/priority/delete'
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="提案名称:">
            <el-input v-model="priorityData.proposalName" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="案件号:">
            <el-input v-model="priorityData.caseNumber" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="申请号:">
            <el-input v-model="priorityData.applicationNumber" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="案件状态:">
            <el-select v-model="priorityData.caseStatus" :disabled="true" placeholder="后台配置选择">
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="案件名称:">
            <el-input v-model="priorityData.caseName" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="申请类型:">
            <el-select v-model="priorityData.applicationType" :disabled="true" placeholder="后台配置选择">
              <el-option label="后台配置选择" value=""></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="处理事项:">
            <el-input v-model="priorityData.processItem" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="发文日:">
            <el-input v-model="priorityData.issueDate" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="官方期限:">
            <el-input v-model="priorityData.officialDeadline" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="内部期限:">
            <el-input v-model="priorityData.internalDeadline" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="业务人员:">
            <el-select v-model="priorityData.businessPerson" :disabled="true" placeholder="后台配置选择">
              <el-option label="后台配置选择" value=""></el-option>
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="递交类型:">
            <el-select v-model="priorityData.submissionType" placeholder="请选择" :disabled="true">
        ...
        <el-col :span="6">
          <el-form-item label="联系电话:">
            <el-input v-model="priorityData.contactPhone" placeholder="请输入联系电话" :disabled="true"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="电子邮箱:">
            <el-input v-model="priorityData.email" placeholder="请输入电子邮箱" :disabled="true"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="交存请求:">
            <el-checkbox v-model="priorityData.depositRequest" disabled>请求使用优先权文件数字接入服务(DAS)</el-checkbox>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="查询请求:">
            <el-checkbox v-model="priorityData.queryRequest" disabled>请求使用优先权文件数字接入服务(DAS)进行查询获取</el-checkbox>
          </el-form-item>
        </el-col>
        </el-row>
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="客户名称:">
            <el-input v-model="priorityData.clientName" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="技术主导:">
            <el-input v-model="priorityData.techLead" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item>
        ...
        <el-col :span="6">
          <el-form-item label="双报案件:">
            <el-input v-model="priorityData.doubleReport" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="案件id:">
            <el-input v-model="priorityData.caseId" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="处理事项id:">
            <el-input v-model="priorityData.processItemId" :disabled="true" placeholder="单行输入"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
  ...
  <el-tabs v-model="activeTab" @tab-change="handleTabChange">
    <el-tab-pane label="请求书" name="request">
      ...
      <el-row :gutter="20" style="margin: 10px 0 20px;">
        ...
        <el-col :span="6">
           <el-select v-model="priorityData.submissionType" placeholder="请选择" style="width: 100%;">
             <el-option label="电子递交" value="电子递交"></el-option>
             <el-option label="纸质递交" value="纸质递交"></el-option>
      ...
      <el-card style="margin-bottom: 10px;">
        ...
        <el-form label-width="auto">
          <h4 style="margin-top: 20px; margin-bottom: 10px; font-weight: bold; color: #333;">
            专利法实施细则第31条第1款 或 专利合作条约实施细则第17条 的规定，向国家知识产权局(以下项目二选一)提出:
          </h4>
          ...
          <el-form-item style="margin-top: 15px;">
            <el-checkbox v-model="priorityData.depositRequest" style="display: block; margin-bottom: 10px;">
    ...
    <el-tab-pane label="待转档文件" name="pending">
      ...
      <div style="margin-bottom: 20px;">
        <input ref="pendingFileInput" type="file" multiple style="display: none" @change="onPendingFileChange" />
        <el-button @click="onStartXmlConversion">启动转档XML</el-button>
        <span v-if="selectedPendingFiles.length" style="margin-left: 10px;">已选择 {{ selectedPendingFiles.length }} 个文件</span>
      </div>
